...
class CustomWorkerBehavior : public xthread::WorkerInterface {
  ...
  void scheduler_prologue(xthread::Worker& w) override {
  ...
  void scheduler_epilogue(xthread::Worker& w, std::exception_ptr) override {
...
int main() {
  ...
  xthread::Executor executor(4, xthread::make_worker_interface<CustomWorkerBehavior>());
  return 0;
}
