/**
 ...
 we disable this behavior by calling `xthread::Subflow::retain(true)`.
...
int main() {
    ...
    xthread::Taskflow taskflow("visitor");
    ...
    taskflow.for_each_task([](xthread::Task task) {
        std::cout << "task " << task.name() << " [type=" << xthread::to_string(task.type()) << "]\n";
        // traverse it's successor
        task.for_each_successor([](xthread::Task successor) {
            std::cout << "  -> successor   task " << successor.name() << '\n';
        });
        // traverse it's predecessor
        task.for_each_predecessor([](xthread::Task predecessor) {
        ...
        task.for_each_subflow_task([](xthread::Task stask) {
            ...
            // traverse it's successor
            stask.for_each_successor([](xthread::Task successor) {
                std::cout << "    -> successor   task " << successor.name() << '\n';
            });
            // traverse it's predecessor
            stask.for_each_predecessor([](xthread::Task predecessor) {
                std::cout << "    <- predecessor task " << predecessor.name() << '\n';
            });
        });
    });
