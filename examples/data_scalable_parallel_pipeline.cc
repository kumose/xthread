...
int main() {
  ...
  const size_t num_lines = 4;
    ...
    //1. How can I put a placeholder in the first pipe, i.e. [] (void, xthread::Pipeflow&) in order to match the pipe vector?
    auto pipe_callable1 = [] (xthread::Pipeflow& pf) mutable -> int {
        if(pf.token() == 5) {
          pf.stop();
          return 0;
        }
    ...
    auto pipe_callable2 = [] (int input, xthread::Pipeflow& pf) mutable -> float {
        return input + 1.0;
    };
    auto pipe_callable3 = [] (float input, xthread::Pipeflow& pf) mutable -> int {
        return input + 1;
    };
  ...
  pipes.emplace_back(xthread::make_scalable_datapipe<void, int>(xthread::PipeType::SERIAL, pipe_callable1));
  pipes.emplace_back(xthread::make_scalable_datapipe<int, float>(xthread::PipeType::SERIAL, pipe_callable2));
  pipes.emplace_back(xthread::make_scalable_datapipe<float, int>(xthread::PipeType::SERIAL, pipe_callable3));

  // create a pipeline of four parallel lines using the given vector of pipes
  xthread::ScalablePipeline<decltype(pipes)::iterator> pl(num_lines, pipes.begin(), pipes.end());
  ...
  pipes.emplace_back(xthread::make_scalable_datapipe<int, float>(xthread::PipeType::SERIAL, pipe_callable1));
  pipes.emplace_back(xthread::make_scalable_datapipe<float, int>(xthread::PipeType::SERIAL, pipe_callable1));
