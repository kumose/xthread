...
int main(int argc, char* argv[]) {
  ...
  // gpu data
  auto d_keys = xthread::cuda_malloc_shared<int>(N);
  ...
  auto p = xthread::cudaDefaultExecutionPolicy{};
  
  auto beg = std::chrono::steady_clock::now();
  xthread::cudaStream s;
  auto bufsz = xthread::cuda_sort_buffer_size<decltype(p), int>(N);
  xthread::cudaDeviceVector<std::byte> buf(bufsz);
  xthread::cuda_sort(p, d_keys, d_keys+N, xthread::cuda_less<int>{}, buf.data());
  s.synchronize();
