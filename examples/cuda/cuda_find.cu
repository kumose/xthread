...
int main(int argc, char* argv[]) {
  ...
  // gpu data
  auto gdata = xthread::cuda_malloc_shared<int>(N);
  auto gfind = xthread::cuda_malloc_shared<unsigned>(1);
  ...
  // delete the memory
  xthread::cuda_free(gdata);
  xthread::cuda_free(gfind);
