...
size_t spawn_async(size_t N, xthread::Runtime& rt) {
  ...
  rt.silent_async([N, &res1](xthread::Runtime& rt1){ res1 = spawn_async(N-1, rt1); });
...
size_t fibonacci_async(size_t N) {
  size_t res;
  get_executor().async([N, &res](xthread::Runtime& rt){ res = spawn_async(N, rt); }).get();
  return res;
}
