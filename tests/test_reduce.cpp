...
#include <xthread/algorithm/reduce.hpp>
...
void reduce_by_index_sum(unsigned W) {
  ...
  for(size_t n=1; n<vec.size(); n++) {
    for(size_t c : {0, 1, 3, 7, 99}) {
      ...
      int sum = 10;
      int sol = 10;
      xthread::IndexRange<size_t> range;
      ...
      ptask = taskflow.reduce_by_index(
        std::ref(range),
        sol,
        [&](xthread::IndexRange<size_t> subrange, std::optional<int> running_total){
  ...
  for (int tc = 1; tc < 16; tc++) {
    for(size_t c : {0, 1, 3, 7, 99}) {
      ...
      taskflow.reduce(range.begin(), range.end(), result, 
        ...
        xthread::StaticPartitioner(c, [&](auto&& task) {
  ...
  for (int tc = 1; tc < 16; tc++) {
    for(size_t c : {0, 1, 3, 7, 99}) {
      ...
      taskflow.reduce(range.begin(), range.end(), result, 
        ...
        xthread::DynamicPartitioner(c, [&](auto&& task) {
          GetThreadSpecificContext() = tc;
          task();
          GetThreadSpecificContext() = 0;
        })
...
void silent_async(unsigned W) {
  ...
  for(size_t n=1; n<vec.size(); n++) {
    ...
    executor.silent_async(xthread::make_reduce_task(
...
void silent_dependent_async(unsigned W) {
  ...
  for(size_t n=1; n<vec.size(); n++) {
    ...
    executor.silent_dependent_async(xthread::make_reduce_task(
      beg, end, pmin, [](int& l, int& r){
      return std::min(l, r);
    }));
