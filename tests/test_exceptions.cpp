...
void joined_subflow_1(unsigned W) {
  ...
  taskflow.emplace([&] (xthread::Subflow& sf0) {
    for (int i = 0; i < 100; ++i) {
      sf0.emplace([&] (xthread::Subflow& sf1) {
...
void joined_subflow_3(unsigned N) {
  ...
  // implicit join
  taskflow.emplace([&](xthread::Subflow& sf) {
    xthread::Task W = sf.emplace([&](){ ++num_tasks; });
    xthread::Task X = sf.emplace([&](){ ++num_tasks; throw std::runtime_error("x"); });
    xthread::Task Y = sf.emplace([&](){ ++num_tasks; });
    xthread::Task Z = sf.emplace([&](){ ++num_tasks; });
    W.precede(X);
    X.precede(Y);
    Y.precede(Z);
  });
  ...
  taskflow.emplace([&](xthread::Subflow& sf) {
    xthread::Task W = sf.emplace([&](){ ++num_tasks; });
    xthread::Task X = sf.emplace([&](){ ++num_tasks; throw std::runtime_error("y"); });
    xthread::Task Y = sf.emplace([&](){ ++num_tasks; });
    xthread::Task Z = sf.emplace([&](){ ++num_tasks; });
    W.precede(X);
    X.precede(Y);
...
void nested_subflow(unsigned N) {
  ...
  // level 1
  taskflow.emplace([&](xthread::Subflow& sf1) {
    xthread::Task V1 = sf1.emplace([&num_tasks](){ ++num_tasks; }).name("V1");
    xthread::Task W1 = sf1.emplace([&num_tasks](){ ++num_tasks; }).name("W1");
    
    // level 2
    xthread::Task X1 = sf1.emplace([&num_tasks](xthread::Subflow& sf2){
      ++num_tasks; 

      xthread::Task V2 = sf2.emplace([&num_tasks](){ ++num_tasks; }).name("V2");
      xthread::Task W2 = sf2.emplace([&num_tasks](){ ++num_tasks; }).name("W2");
      
      // level 3
      xthread::Task X2 = sf2.emplace([&num_tasks](xthread::Subflow& sf3) {
        ++num_tasks;

        xthread::Task V3 = sf3.emplace([&num_tasks](){ ++num_tasks; }).name("V3");
        xthread::Task W3 = sf3.emplace([&num_tasks](){ ++num_tasks; }).name("W3");

        // level 4
        xthread::Task X3 = sf3.emplace([&num_tasks](xthread::Subflow& sf4){
          ++num_tasks;

          xthread::Task V4 = sf4.emplace([&num_tasks](){ ++num_tasks; }).name("V4");
          xthread::Task W4 = sf4.emplace([&num_tasks](){ ++num_tasks; }).name("W4");
          xthread::Task X4 = sf4.emplace([&num_tasks](){
            ++num_tasks; 
            throw std::runtime_error("x");
          }).name("X4 (throw)");
          xthread::Task Y4 = sf4.emplace([&num_tasks](){ ++num_tasks; }).name("Y4");
          xthread::Task Z4 = sf4.emplace([&num_tasks](){ ++num_tasks; }).name("Z4");
        ...
        xthread::Task Y3 = sf3.emplace([&num_tasks](){ ++num_tasks; }).name("Y3");
        xthread::Task Z3 = sf3.emplace([&num_tasks](){ ++num_tasks; }).name("Z3");
      ...
      xthread::Task Y2 = sf2.emplace([&num_tasks](){ ++num_tasks; }).name("Y2");
      xthread::Task Z2 = sf2.emplace([&num_tasks](){ ++num_tasks; }).name("Z2");
    ...
    xthread::Task Y1 = sf1.emplace([&num_tasks](){ ++num_tasks; }).name("Y1");
    xthread::Task Z1 = sf1.emplace([&num_tasks](){ ++num_tasks; }).name("Z1");
  ...
  num_tasks = 0;
  executor.async([&](xthread::Runtime&){
  ...
  num_tasks = 0;
  executor.silent_async([&](xthread::Runtime&){
    //REQUIRE_THROWS_WITH_AS(rt.corun(taskflow), "x", std::runtime_error);
    REQUIRE_THROWS_WITH_AS(executor.corun(taskflow), "x", std::runtime_error);
  });
  executor.wait_for_all(); 
...
void executor_corun_1(unsigned W) {
  ...
  xthread::Executor executor(W);
  xthread::Taskflow taskflow1;
  xthread::Taskflow taskflow2;
  ...
  for(size_t i=0; i<100; i++) {
    taskflow1.emplace([](xthread::Subflow& sf){
...
void runtime_corun_1(unsigned W) {
  ...
  auto task = taskflow2.emplace([&](xthread::Runtime&){
    REQUIRE_THROWS_WITH_AS(executor.corun(taskflow1), "x", std::runtime_error);
  });
  executor.run(taskflow2).get();
...
void runtime_corun_2(unsigned W) {
  ...
  // uncaught corun exception propagates to the topology 
  xthread::Taskflow taskflow2;
  taskflow2.emplace([&](xthread::Runtime&){
    executor.corun(taskflow);
  });
  REQUIRE_THROWS_WITH_AS(executor.run(taskflow2).get(), "x", std::runtime_error);
  REQUIRE(counter == 2);
  ...
  // catch corun exception directly
  xthread::Taskflow taskflow3;
  taskflow3.emplace([&](xthread::Runtime&){
    REQUIRE_THROWS_WITH_AS(executor.corun(taskflow), "x", std::runtime_error);
  });
  executor.run(taskflow3).get();
  REQUIRE(counter == 4);
...
void async_with_runtime(unsigned W) {
  ...
  for(size_t i=0; i<1024; i++) {
    futures.emplace_back(executor.async([](xthread::Runtime&){
      throw std::runtime_error("x");
    }));
  }
...
void dependent_async_with_runtime(unsigned W) {
  ...
  for(size_t i=0; i<1024; i++) {
    auto [t, f] = executor.dependent_async([](xthread::Runtime&){
      throw std::runtime_error("x");
    });
  }
  ...
  // silently caught by the task
  executor.silent_dependent_async([](xthread::Runtime&){
    throw std::runtime_error("x");
  });
...
void runtime_async_task(unsigned W) {
  ...
  // runtime async
  auto A = taskflow.emplace([](xthread::Runtime& rt){
  ...
  flag = 0;
  taskflow.clear();
  A = taskflow.emplace([&](xthread::Runtime& rt){
...
void semaphore1(unsigned W) {
  ...
  xthread::Task B = taskflow.emplace([](){ throw std::runtime_error("exception"); });
  xthread::Task C = taskflow.emplace([](){});
  xthread::Task D = taskflow.emplace([](){});
