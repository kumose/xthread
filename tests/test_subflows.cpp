...
void joined_subflow(unsigned W) {
  ...
  SUBCASE("Trivial") {
    ...
    xthread::Task subflow3, subflow3_;
    //std::future<int> fu3, fu3_;
    std::atomic<int> fu3v{0}, fu3v_{0};

    // empty flow
    auto subflow1 = tf.emplace([&] (xthread::Subflow& fb) {
      fu3v++;
      fb.join();
    }).name("subflow1");

    // nested empty flow
    auto subflow2 = tf.emplace([&] (xthread::Subflow& fb) {
      fu3v++;
      fb.emplace([&] (xthread::Subflow& fb2) {
        fu3v++;
        fb2.emplace( [&] (xthread::Subflow& fb3) {
          fu3v++;
          fb3.join();
        }).name("subflow2_1_1");
      }).name("subflow2_1");
    ...
    subflow3 = tf.emplace([&] (xthread::Subflow& fb) {
      ...
      subflow3_ = fb.emplace([&] (xthread::Subflow& fb2) {
        REQUIRE(fu3v_ == 3);
        fu3v++;
        fu3v_++;
        //return 200;
  ...
  SUBCASE("Complex") {
    ...
    auto B = tf.emplace([&count, &data, &sum](xthread::Subflow& fb){
      ...
      auto n = fb.emplace([&count](xthread::Subflow& fb2){
...
//// --------------------------------------------------------
//void detach_spawn(const int max_depth, std::atomic<int>& counter, int depth, xthread::Subflow& subflow)  {
//  if(depth < max_depth) {
//    counter.fetch_add(1, std::memory_order_relaxed);
//    subflow.emplace([&, max_depth, depth=depth+1](xthread::Subflow& sfl){
//      detach_spawn(max_depth, counter, depth, sfl); }
//    );
//    subflow.emplace([&, max_depth, depth=depth+1](xthread::Subflow& sfr){
...
//}
//
//void join_spawn(const int max_depth, std::atomic<int>& counter, int depth, xthread::Subflow& subflow)  {
//  if(depth < max_depth) {
//    counter.fetch_add(1, std::memory_order_relaxed);
//    subflow.emplace([&, max_depth, depth=depth+1](xthread::Subflow& sfl){
...
//
//void mix_spawn(
//  const int max_depth, std::atomic<int>& counter, int depth, xthread::Subflow& subflow
//) {
//
//  if(depth < max_depth) {
...
//      xthread::Taskflow tf;
//      tf.emplace([&](xthread::Subflow& subflow){
...
// Testcase: FibSubflow
// --------------------------------------------------------
int fibonacci_spawn(int n, xthread::Subflow& sbf) {
  if (n < 2) return n;
  int res1, res2;
  sbf.emplace([&res1, n] (xthread::Subflow& sbfl) { res1 = fibonacci_spawn(n - 1, sbfl); } );
  sbf.emplace([&res2, n] (xthread::Subflow& sbfr) { res2 = fibonacci_spawn(n - 2, sbfr); } );
...
void fibonacci(size_t W) {
  ...
  taskflow.emplace([&res, N] (xthread::Subflow& sbf) {
...
void multiple_subflow_runs(unsigned W) {
  ...
  auto B = taskflow.emplace([&](xthread::Subflow& subflow){
    count ++;
  ...
  std::list<xthread::Future<void>> fu_list;
...
void bit_state(unsigned W) {
  ...
  auto task = taskflow.emplace([](xthread::Subflow& sf){
