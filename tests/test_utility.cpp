...
#include <xthread/utility/os.hpp>
//#include <xthread/utility/object_pool.hpp>
#include <xthread/utility/small_vector.hpp>
#include <xthread/utility/uuid.hpp>
#include <xthread/utility/iterator.hpp>
#include <xthread/utility/math.hpp>
#include <xthread/utility/traits.hpp>
...
TEST_CASE("SmallVector" * doctest::timeout(300)) {
  ...
  //SUBCASE("constructor")
  {
    xthread::SmallVector<int> vec1;
    REQUIRE(vec1.size() == 0);
    REQUIRE(vec1.empty() == true);

    xthread::SmallVector<int, 4> vec2;
  ...
  {
    for(int N=0; N<=65536; N = (N ? N << 1 : 1)) {
      xthread::SmallVector<int> vec1(N);
      for(auto& item : vec1) {
        item = N;
      }

      xthread::SmallVector<int> vec2(vec1);
      REQUIRE(vec1.size() == N);
      REQUIRE(vec2.size() == N);
  ...
  //SUBCASE("move_constructor")
  {
    for(int N=0; N<=65536; N = (N ? N << 1 : 1)) {
      ...
      xthread::SmallVector<int> vec2(std::move(vec1));
      REQUIRE(vec1.size() == 0);
      REQUIRE(vec1.empty() == true);
      REQUIRE(vec2.size() == N);
  ...
  for(int beg=-50; beg<=50; ++beg) {
    for(int end=-50; end<=50; ++end) {
      if(beg < end) {   // positive step
        for(int s=1; s<=50; s++) {
          REQUIRE((xthread::distance(beg, end, s) == count(beg, end, s)));
        }
      }
      else {            // negative step
        for(int s=-1; s>=-50; s--) {
...
void test_threaded_uuid(size_t N) {
  ...
  std::vector<xthread::UUID> uuids(65536);
...
TEST_CASE("ObjectPool.Sequential" * doctest::timeout(300)) {
  ...
  for(unsigned w=1; w<=4; w++) {
    ...
    xthread::ObjectPool<Poolable> pool(w);
...
TEST_CASE("RefWrapper" * doctest::timeout(300)) {
  ...
  static_assert(std::is_same<
    xthread::unwrap_reference_t<int>, int
  >::value, "");
  ...
  static_assert(std::is_same<
    xthread::unwrap_reference_t<std::reference_wrapper<int>>, int&
  >::value, "");

  static_assert(std::is_same<
    xthread::unwrap_reference_t<std::reference_wrapper<std::reference_wrapper<int>>>,
    std::reference_wrapper<int>&
  >::value, "");

  static_assert(std::is_same<
    xthread::unwrap_ref_decay_t<int>, int
  >::value, "");
  ...
  static_assert(std::is_same<
    xthread::unwrap_ref_decay_t<std::reference_wrapper<int>>, int&
  >::value, "");

  static_assert(std::is_same<
    xthread::unwrap_ref_decay_t<std::reference_wrapper<std::reference_wrapper<int>>>,
    std::reference_wrapper<int>&
  >::value, "");
...
//
//  SUBCASE("func1") {
//    using func1_traits = xthread::function_traits<decltype(func1)>;
//    static_assert(std::is_same<func1_traits::return_type, void>::value, "");
//    static_assert(func1_traits::arity == 0, "");
//  }
//
//  SUBCASE("func2") {
//    using func2_traits = xthread::function_traits<decltype(func2)>;
...
//      return 1;
//    };
//    using lambda1_traits = xthread::function_traits<decltype(lambda1)>;
...
//    };
//    using lambda2_traits = xthread::function_traits<decltype(lambda2)>;
...
//    };
//    using foo_traits = xthread::function_traits<foo>;
...
//    using ft1 = xthread::function_traits<std::function<void()>>;
//    static_assert(std::is_same<ft1::return_type, void>::value, "");
//    static_assert(ft1::arity == 0, "");
//
//    using ft2 = xthread::function_traits<std::function<int(int&, double&&)>&>;
...
//
//    using ft3 = xthread::function_traits<std::function<int(int&, double&&)>&&>;
...
//
//    using ft4 = xthread::function_traits<const std::function<void(int)>&>;
...
TEST_CASE("NextPow2") {
  ...
  static_assert(xthread::next_pow2(0u) == 1);
  static_assert(xthread::next_pow2(1u) == 1);
  static_assert(xthread::next_pow2(100u) == 128u);
  static_assert(xthread::next_pow2(245u) == 256u);
  static_assert(xthread::next_pow2(512u) == 512u);
  static_assert(xthread::next_pow2(513u) == 1024u);

  REQUIRE(xthread::next_pow2(0u) == 1u);
  REQUIRE(xthread::next_pow2(2u) == 2u);
  REQUIRE(xthread::next_pow2(1u) == 1u);
  REQUIRE(xthread::next_pow2(33u) == 64u);
  REQUIRE(xthread::next_pow2(100u) == 128u);
  REQUIRE(xthread::next_pow2(211u) == 256u);
  REQUIRE(xthread::next_pow2(23u) == 32u);
  REQUIRE(xthread::next_pow2(54u) == 64u);

  uint64_t z = 0;
  uint64_t a = 1;
  REQUIRE(xthread::next_pow2(z) == 1);
  REQUIRE(xthread::next_pow2(a) == a);
  REQUIRE(xthread::next_pow2((a<<5)  + 0) == (a<<5));
  REQUIRE(xthread::next_pow2((a<<5)  + 1) == (a<<6));
  REQUIRE(xthread::next_pow2((a<<32) + 0) == (a<<32));
  REQUIRE(xthread::next_pow2((a<<32) + 1) == (a<<33));
  REQUIRE(xthread::next_pow2((a<<41) + 0) == (a<<41));
  REQUIRE(xthread::next_pow2((a<<41) + 1) == (a<<42));

  REQUIRE(xthread::is_pow2(0) == false);
  REQUIRE(xthread::is_pow2(1) == true);
  REQUIRE(xthread::is_pow2(2) == true);
  REQUIRE(xthread::is_pow2(3) == false);
  REQUIRE(xthread::is_pow2(0u) == false);
  REQUIRE(xthread::is_pow2(1u) == true);
  REQUIRE(xthread::is_pow2(54u) == false);
  REQUIRE(xthread::is_pow2(64u) == true);
}
...
TEST_CASE("CTZ") {
  REQUIRE(xthread::ctz<uint32_t>(0b00000001) == 0);
  REQUIRE(xthread::ctz<uint32_t>(0b00000010) == 1);
  REQUIRE(xthread::ctz<uint32_t>(0b00000100) == 2);
  REQUIRE(xthread::ctz<uint32_t>(0b10000000) == 7);

  REQUIRE(xthread::ctz<uint64_t>(0b00000001ULL) == 0);
  REQUIRE(xthread::ctz<uint64_t>(0b00000010ULL) == 1);
  REQUIRE(xthread::ctz<uint64_t>(0b00000100ULL) == 2);
  REQUIRE(xthread::ctz<uint64_t>(0x8000000000000000ULL) == 63);

  //REQUIRE(xthread::ctz<uint32_t>(0) == 32); // Undefined behavior, doesn't work for Windows
  REQUIRE(xthread::ctz<uint32_t>(0xFFFFFFFF) == 0);
  REQUIRE(xthread::ctz<uint32_t>(0x00010000) == 16);
  REQUIRE(xthread::ctz<uint32_t>(0x80000000) == 31);

  //REQUIRE(xthread::ctz<uint64_t>(0) == 64); // Undefined behavior, doesn't work for Windows
  REQUIRE(xthread::ctz<uint64_t>(0xFFFFFFFFFFFFFFFFULL) == 0);
  REQUIRE(xthread::ctz<uint64_t>(0x0000000100000000ULL) == 32);
  REQUIRE(xthread::ctz<uint64_t>(0x0000000000008000ULL) == 15);
  REQUIRE(xthread::ctz<uint64_t>(0x4000000000000000ULL) == 62);
}
...
TEST_CASE("Coprimes") {
  ...
  static_assert(xthread::coprime(1) == 1);
  static_assert(xthread::coprime(2) == 1);
  static_assert(xthread::coprime(3) == 2);
  static_assert(xthread::coprime(4) == 3);
  static_assert(xthread::coprime(5) == 4);
  static_assert(xthread::coprime(6) == 5);
  static_assert(xthread::coprime(7) == 6);
  static_assert(xthread::coprime(8) == 7);
  static_assert(xthread::coprime(9) == 8);
  static_assert(xthread::coprime(10) == 9);
  static_assert(xthread::coprime(11) == 10);
  static_assert(xthread::coprime(12) == 11);
  static_assert(xthread::coprime(13) == 12);
  static_assert(xthread::coprime(14) == 13);
  static_assert(xthread::coprime(15) == 14);
  static_assert(xthread::coprime(16) == 15);
  static_assert(xthread::coprime(17) == 16);
  static_assert(xthread::coprime(18) == 17);
  static_assert(xthread::coprime(19) == 18);
  static_assert(xthread::coprime(20) == 19);

  constexpr auto coprime_table = xthread::make_coprime_lut<51>();
  ...
  for (size_t i = 1; i <= 50; ++i) {
    ...
    REQUIRE(xthread::coprime(i) == coprime_table[i]);
    
    // randomly generate a coprime
    auto v = ::rand() % 1048 + 2;
    auto c = xthread::coprime(v);
...
TEST_CASE("FloorLog2") {
  ...
  REQUIRE(xthread::floor_log2(1u) == 0);
  REQUIRE(xthread::floor_log2(2u) == 1);
  REQUIRE(xthread::floor_log2(4u) == 2);
  REQUIRE(xthread::floor_log2(8u) == 3);
  REQUIRE(xthread::floor_log2(16u) == 4);
  REQUIRE(xthread::floor_log2(32u) == 5);
  REQUIRE(xthread::floor_log2(64u) == 6);
  REQUIRE(xthread::floor_log2(128u) == 7);
  REQUIRE(xthread::floor_log2(256u) == 8);

  // Test non-powers of 2 (floor log2)
  REQUIRE(xthread::floor_log2(3u) == 1);
  REQUIRE(xthread::floor_log2(5u) == 2);
  REQUIRE(xthread::floor_log2(6u) == 2);
  REQUIRE(xthread::floor_log2(7u) == 2);
  REQUIRE(xthread::floor_log2(9u) == 3);
  REQUIRE(xthread::floor_log2(10u) == 3);
  REQUIRE(xthread::floor_log2(15u) == 3);
  REQUIRE(xthread::floor_log2(17u) == 4);
  REQUIRE(xthread::floor_log2(31u) == 4);
  REQUIRE(xthread::floor_log2(33u) == 5);
  
  // Test large values
  REQUIRE(xthread::floor_log2(1023u) == 9);
  REQUIRE(xthread::floor_log2(1024u) == 10);
  REQUIRE(xthread::floor_log2(1025u) == 10);
  REQUIRE(xthread::floor_log2(std::numeric_limits<uint32_t>::max()) == 31);
  REQUIRE(xthread::floor_log2(std::numeric_limits<uint64_t>::max()) == 63);
}
