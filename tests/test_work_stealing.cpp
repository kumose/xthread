...
void subflow_starvation(size_t W) {
  ...
  taskflow.emplace([&](xthread::Subflow& subflow){
    ...
    // fork
    for(size_t b=0; b<B; b++) {
      xthread::Task task = subflow.emplace([&](){
        // record the threads
        {
          std::scoped_lock lock(mutex);
          set.insert(executor.this_worker_id());
...
void embarrasing_starvation(size_t W) {
  ...
  xthread::Taskflow taskflow, parent;
  xthread::Executor executor(W);
...
void skewed_starvation(size_t W) {
  ...
  xthread::Task parent = taskflow.emplace([&](){
    set.clear();
    count.store(0, std::memory_order_relaxed);
    stop.store(false, std::memory_order_relaxed);
  }).name("root");

  xthread::Task left, right;
...
void nary_starvation(size_t W) {
  ...
  for(size_t w=0; w<W; w++) {
    ...
    for(size_t i=0; i<N; i++) {
      xthread::Task task = taskflow.emplace([&, w, p, i](){
...
void wavefront_starvation(size_t W) {
  ...
  std::vector<std::vector<xthread::Task>> G;
  G.resize(W);
