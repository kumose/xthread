...
TEST_CASE("SimplePriority.Sequential" * doctest::timeout(300)) {
  ...
  REQUIRE(B.priority() == xthread::TaskPriority::HIGH);
  REQUIRE(C.priority() == xthread::TaskPriority::HIGH);
  REQUIRE(D.priority() == xthread::TaskPriority::HIGH);

  B.priority(xthread::TaskPriority::HIGH);
  C.priority(xthread::TaskPriority::LOW);
  D.priority(xthread::TaskPriority::NORMAL);

  REQUIRE(B.priority() == xthread::TaskPriority::HIGH);
  REQUIRE(C.priority() == xthread::TaskPriority::LOW);
  REQUIRE(D.priority() == xthread::TaskPriority::NORMAL);
...
TEST_CASE("RandomPriority.Sequential" * doctest::timeout(300)) {
  ...
  const auto MAX_P = static_cast<unsigned>(xthread::TaskPriority::MAX);
  ...
  for(size_t i=0; i<10000; i++) {
    ...
    taskflow.emplace([p, &counters](){ counters[p]++; })
            .priority(static_cast<xthread::TaskPriority>(p))
            .succeed(beg)
            .precede(end);
    priorities[p]++;
  }
