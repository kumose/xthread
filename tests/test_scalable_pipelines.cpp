...
void scalable_pipeline(size_t num_lines, size_t num_pipes) {
  ...
  for(size_t i=0; i<num_pipes; i++) {
    pipes.emplace_back(xthread::PipeType::SERIAL, [&](xthread::Pipeflow& pf) mutable {
  ...
  xthread::ScalablePipeline spl(num_lines, pipes.begin(), pipes.end());
...
void scalable_pipeline_reset(size_t num_lines, size_t num_pipes) {
  ...
  xthread::ScalablePipeline<typename decltype(pipes)::iterator> spl(num_lines);
...
void scalable_pipeline_lines_reset(size_t num_lines, size_t num_pipes) {
  ...
  std::vector<xthread::Pipe<>> pipes;
  xthread::ScalablePipeline<typename decltype(pipes)::iterator> spl;
...
void ifelse_spipeline(size_t L, unsigned w) {
  ...
  std::vector<xthread::Pipe<>> pipes;
  xthread::ScalablePipeline<typename decltype(pipes)::iterator> pl;
  ...
  for(size_t N = 1; N < maxN; ++N) {
    ...
    // pipe 1
    pipes.emplace_back(xthread::PipeType::SERIAL, [&, N](auto& pf){
        if(pf.token() == N) {
          pf.stop();
          return;
        }
      ...
      // pipe 2
    pipes.emplace_back(xthread::PipeType::PARALLEL, [&](auto& pf){
    ...
    // pipe 3
    pipes.emplace_back(xthread::PipeType::SERIAL, [&](auto& pf){
...
void spipeline_in_spipeline(size_t L, unsigned w, unsigned subL) {
  ...
  for (size_t N = 1; N < maxN; ++N) {
    for(size_t subN = 1; subN < maxsubN; ++subN) {
      ...
      // begin of pipe 1 -----------------------------
      pipes.emplace_back(xthread::PipeType::SERIAL, [&, N, subN, subL](auto& pf) mutable {
        if(j1 == N) {
          pf.stop();
          return;
        }
        ...
        std::vector<xthread::Pipe<>> subpipes;
        xthread::ScalablePipeline<typename decltype(subpipes)::iterator> subpl;

        // subpipe 1
        subpipes.emplace_back(xthread::PipeType::SERIAL, [&, subN](auto& subpf) mutable {
            if(subj1 == subN) {
              subpf.stop();
              return;
            }
        ...
        // subpipe 2
        subpipes.emplace_back(xthread::PipeType::PARALLEL, [&, subN](auto& subpf) mutable {
            REQUIRE(subj2++ < subN);
            REQUIRE(subpf.token() % subL == subpf.line());
            REQUIRE(
      ...
      //begin of pipe 2 ---------------------------
      pipes.emplace_back(xthread::PipeType::PARALLEL, [&, subN, subL](auto& pf) mutable {
      ...
      // begin of pipe 4 ---------------------------
      pipes.emplace_back(xthread::PipeType::SERIAL, [&, subN](auto& pf) mutable {
        int res = std::accumulate(
          source[j4].begin(),
...
void snig_spipeline(size_t L, unsigned w) {
  ...
  for(auto&& NUM_DEVICE: NUM_DEVICES) {
    ...
    std::vector<xthread::Task> dev_ends(NUM_DEVICE);
    for(auto&& dev_end: dev_ends) {
      dev_end = taskflow.emplace([](){}).name("dev_end");
    }

    std::vector<xthread::Task> first_fetches(NUM_DEVICE);
    std::vector<xthread::Task> fetches(NUM_DEVICE);
    std::vector<std::vector<xthread::Pipe<>>> pipes(NUM_DEVICE);

    // for type
    using pipeline_it = std::vector<xthread::Pipe<>>::iterator;

    std::vector<xthread::Task> module_of_pipelines(NUM_DEVICE);

    std::vector<xthread::ScalablePipeline<pipeline_it>> pipelines(NUM_DEVICE);
    ...
    for(size_t dev = 0; dev < NUM_DEVICE; ++dev) {
      ...
      // pipe 2
      pipes[dev].emplace_back(
        xthread::PipeType::PARALLEL, [&, dev, BATCH_SIZE](auto& pf) mutable {
      ...
      // pipe 3
      pipes[dev].emplace_back(
        xthread::PipeType::SERIAL, [&, dev, BATCH_SIZE](auto& pf) mutable {
...
void spawn(
  ...
  std::vector<std::vector<xthread::Pipe<>>>& pipes,
  std::vector<xthread::ScalablePipeline<typename std::vector<xthread::Pipe<>>::iterator>>& spls,
  size_t& counter
) {

  // construct pipes
  for(size_t p = 0; p < NUM_PIPES; ++p) {
    pipes[r].emplace_back(xthread::PipeType::SERIAL, [&, maxN, r](xthread::Pipeflow& pf) mutable {
  ...
  if(r + 1 < NUM_RECURS) {
    auto spawn_t = sf.emplace([&, L, NUM_PIPES, NUM_RECURS, maxN, r](xthread::Subflow& sf2) mutable {
...
void subflow_spipeline(unsigned NUM_RECURS, unsigned w, size_t L) {
  ...
  xthread::Executor executor(w);
  xthread::Taskflow taskflow;
  std::vector<xthread::ScalablePipeline<typename std::vector<xthread::Pipe<>>::iterator>> spls(NUM_RECURS);
  std::vector<std::vector<xthread::Pipe<>>> pipes(NUM_RECURS);
  ...
  auto subflows = taskflow.emplace([&, L, NUM_PIPES, NUM_RECURS, maxN](xthread::Subflow& sf){
...
TEST_CASE("ScalablePipeline.move" * doctest::timeout(300)) {
  ...
  for(size_t i=0; i<N; i++) {
    pipes.emplace_back(xthread::PipeType::SERIAL, [&](xthread::Pipeflow& pf) {
      if (pf.token() == 5) {
        pf.stop();
      }
      else {
