...
TEST_CASE("Runtime.Schedule.ModuleTask" * doctest::timeout(300)) {
  ...
  auto schedule = tf.emplace([&](xthread::Runtime& runtime) {
    value++;
...
TEST_CASE("Runtime.ExternalGraph.Simple" * doctest::timeout(300)) {
  ...
  std::vector<xthread::Taskflow> graphs(N);
  ...
  for(size_t i=0; i<N; i++) {
    ...
    taskflow.emplace([&executor, &res=results[i], &graph=graphs[i]](xthread::Runtime&) mutable {
...
size_t fibonacci(size_t N, xthread::Runtime& rt) {
  ...
  rt.silent_async([N, &res1](xthread::Runtime& rt1){ res1 = fibonacci(N-1, rt1); });
...
size_t fibonacci(size_t T, size_t N) {
  xthread::Executor executor(T);
  size_t res;
  executor.async([N, &res](xthread::Runtime& rt){ res = fibonacci(N, rt); }).get();
  return res;
}
...
size_t fibonacci_swapped(size_t N, xthread::Runtime& rt) {
  ...
  rt.silent_async([N, &res2](xthread::Runtime& rt2){ res2 = fibonacci_swapped(N-2, rt2); });
...
size_t fibonacci_swapped(size_t T, size_t N) {
  xthread::Executor executor(T);
  size_t res;
  executor.async([N, &res](xthread::Runtime& rt){ res = fibonacci_swapped(N, rt); }).get();
  return res;
}
...
void implicit_join(size_t num_threads) {
  ...
  auto B = taskflow.emplace([&](xthread::Runtime& rt){
...
void recursive_implicit_join(size_t num_threads) {
  ...
  auto A = taskflow.emplace([&](){ counter.store(0, std::memory_order_relaxed); });
  auto B = taskflow.emplace([&](xthread::Runtime& rt1){
    // implicitly synchronize all async tasks
    for(size_t i=0; i<N1; i++) {
      rt1.async([&](xthread::Runtime& rt2){
    ...
    for(size_t i=0; i<N1; i++) {
      rt1.silent_async([&](xthread::Runtime& rt2){
        counter.fetch_add(1, std::memory_order_relaxed); 
        // each of the N1 async task spawns N2 async task
        for(size_t j=0; j<N2; j++) {
          rt2.async([&](){
