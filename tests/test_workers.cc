...
class CustomWorkerBehavior : public xthread::WorkerInterface {
  ...
  void scheduler_prologue(xthread::Worker& wv) override {
    _counter++;
  ...
  void scheduler_epilogue(xthread::Worker&, std::exception_ptr) override {
    _counter++;
  }
...
void worker_interface_basics(unsigned W) {
  ...
  {
    xthread::Executor executor(W, xthread::make_worker_interface<CustomWorkerBehavior>(counter, ids));
  }
