  ...
  for(int n1=0; n1<=123456; n1 = n1*2 + 1) {
    for(int n2=0; n2<=123456; n2 = n2*2 + 1) {
      ...
      taskflow.emplace([n1, n2](){
        ...
        // gpu data
        auto da = xthread::cuda_malloc_shared<T>(n1);
        auto db = xthread::cuda_malloc_shared<T>(n2);
        auto dc = xthread::cuda_malloc_shared<T>(n1 + n2);
        ...
        xthread::cuda_merge(policy,
          da, da+n1, db, db+n2, dc, xthread::cuda_less<T>{}, buf
        );
        stream.synchronize();
  ...
  for(int n1=0; n1<=123456; n1 = n1*2 + 1) {
    for(int n2=0; n2<=123456; n2 = n2*2 + 1) {
      ...
      taskflow.emplace([n1, n2](){
        ...
        // gpu data
        auto da_k = xthread::cuda_malloc_shared<T>(n1);
        auto da_v = xthread::cuda_malloc_shared<T>(n1);
        auto db_k = xthread::cuda_malloc_shared<T>(n2);
        auto db_v = xthread::cuda_malloc_shared<T>(n2);
        auto dc_k = xthread::cuda_malloc_shared<T>(n1 + n2);
        auto dc_v = xthread::cuda_malloc_shared<T>(n1 + n2);
