...
TEST_CASE("cuda.version" * doctest::timeout(300) ) {
  REQUIRE(xthread::cuda_get_driver_version() > 0);
  REQUIRE(xthread::cuda_get_runtime_version() > 0);
}

TEST_CASE("cuda.device" * doctest::timeout(300) ) {

  REQUIRE(xthread::cuda_get_num_devices() > 0);
  REQUIRE(xthread::cuda_get_device() >= 0);

  size_t num_devices = xthread::cuda_get_num_devices();

  for(size_t c=0; c<num_devices; c++) {
    xthread::cuda_set_device(c);
    REQUIRE(xthread::cuda_get_device() == c);
    
    for(size_t d=0; d<num_devices; d++) {
      REQUIRE(xthread::cuda_get_device_max_threads_per_block(d) > 0);
      REQUIRE(xthread::cuda_get_device_max_x_dim_per_block(d) > 0);
      REQUIRE(xthread::cuda_get_device_max_y_dim_per_block(d) > 0);
      REQUIRE(xthread::cuda_get_device_max_z_dim_per_block(d) > 0);
      REQUIRE(xthread::cuda_get_device_max_x_dim_per_grid(d) > 0);
      REQUIRE(xthread::cuda_get_device_max_y_dim_per_grid(d) > 0);
      REQUIRE(xthread::cuda_get_device_max_z_dim_per_grid(d) > 0);
      REQUIRE(xthread::cuda_get_device_warp_size(d) > 0);
      REQUIRE(xthread::cuda_get_device_max_shm_per_block(d) > 0);
      REQUIRE(xthread::cuda_get_device_compute_capability_major(d) > 0);
      REQUIRE(xthread::cuda_get_device_compute_capability_minor(d) >= 0);
      REQUIRE_NOTHROW(xthread::cuda_get_device_unified_addressing(d));
    }
  }
  
  // going back to device 0
  xthread::cuda_set_device(0);
}
...
TEST_CASE("cudaStream" * doctest::timeout(300)) {
  ...
  // create a nullstream
  xthread::cudaStream s3(std::move(s1));

  REQUIRE(s1 == nullptr);
  REQUIRE(s3.get() == s2_source);

  // create an empty stream
  xthread::cudaStream s4(nullptr);
  REQUIRE(s4 == nullptr);
...
TEST_CASE("cudaEvent" * doctest::timeout(300)) {
  ...
  xthread::cudaEvent e2(e2_source);
...
TEST_CASE("cudaGraph" * doctest::timeout(300)) {
  ...
  xthread::cudaGraph g2(g2_source);
...
TEST_CASE("cudaGraphExec" * doctest::timeout(300)) {
  ...
  // create a new graph g1 inside
  xthread::cudaGraph g1, g2, g3;
  xthread::cudaGraphExec e1(g1), e2(g2), e3(g3);
