...
TEST_CASE("cuda_reduce.BufferSize") {
  ...
  using P = xthread::cudaExecutionPolicy<32, 3>;
...
void cuda_reduce() {
  ...
  for(int n=0; n<=1234567; n = (n<=100) ? n+1 : n*2 + 1) {
    taskflow.emplace([n](){
      ...
      auto gpu = xthread::cuda_malloc_shared<T>(n);
      auto res = xthread::cuda_malloc_shared<T>(1);
      auto buf = xthread::cuda_malloc_shared<T>(bufsz);
      for(int i=0; i<n; i++) {
        gpu[i] = i;
        gold += i;
      }
      *res = T{1000};  // initial value
      ...
      xthread::cuda_uninitialized_reduce(policy,
        gpu, gpu + n, res, [] __device__ (T a, T b) { return a + b; }, buf
      );
      stream.synchronize();
  ...
  for(int n=0; n<=1234567; n = (n<=100) ? n+1 : n*2 + 1) {
    taskflow.emplace([n](){
      ...
      *res = T{1000};  // initial value
      ...
      // reduce
      xthread::cuda_transform_reduce(policy,
        gpu, gpu + n, res, 
      ...
      // uninitialized reduce
      xthread::cuda_uninitialized_transform_reduce(policy,
        gpu, gpu + n, res, 
        [] __device__ (T a, T b) { return a + b; }, 
        [] __device__ (T a)      { return -a; },
        buf
