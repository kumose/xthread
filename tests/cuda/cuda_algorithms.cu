...
void add2() {
  ...
  for(size_t N=1; N<=(1<<20); N <<= 1) {
    ...
    xthread::cudaDefaultExecutionPolicy p;

    auto input  = xthread::cuda_malloc_shared<T>(N);
    auto output = xthread::cuda_malloc_shared<T>(N);
    for(size_t n=0; n<N; n++) {
      input[n] = 1;
    }

    xthread::cuda_transform(p, input, input + N, output,
...
void transform() {
  ...
  for(unsigned n=1; n<=1234567; n = n*2 + 1) {
    ...
    auto src1 = xthread::cuda_malloc_shared<int>(n);
    auto src2 = xthread::cuda_malloc_shared<int>(n);
    auto dest = xthread::cuda_malloc_shared<int>(n);
...
TEST_CASE("cudaflow.transform" * doctest::timeout(300)) {
  transform<xthread::cudaFlow>();
}

TEST_CASE("capture.transform" * doctest::timeout(300) ) {
  transform<xthread::cudaFlowCapturer>();
}
...
void scan() {
  ...
  for(int N=1; N<=1234567; N = N*2 + 1) {
    ...
    auto data1 = xthread::cuda_malloc_shared<T>(N);
    auto data2 = xthread::cuda_malloc_shared<T>(N);
    auto scan1 = xthread::cuda_malloc_shared<T>(N);
    auto scan2 = xthread::cuda_malloc_shared<T>(N);
    ...
    xthread::cudaDeviceVector<std::byte> temp(
      xthread::cuda_scan_buffer_size<xthread::cudaDefaultExecutionPolicy, T>(N)
    );
      
    xthread::cuda_inclusive_scan(xthread::cudaDefaultExecutionPolicy{},
      data1, data1+N, scan1, xthread::cuda_plus<T>{}, temp.data()
    );
    cudaStreamSynchronize(0);

    xthread::cuda_exclusive_scan(xthread::cudaDefaultExecutionPolicy{},
      data2, data2+N, scan2, xthread::cuda_plus<T>{}, temp.data()
    );
    cudaStreamSynchronize(0);
...
void transform_scan() {
  ...
  for(int N=1; N<=1234567; N = N*2 + 1) {
    ...
    xthread::cuda_transform_inclusive_scan(xthread::cudaDefaultExecutionPolicy{},
      data1, data1+N, scan1, 
      [] __device__ (T a, T b){ return a+b; },
      [] __device__ (T a) { return a*10; },
      temp.data()
    ...
    xthread::cuda_transform_exclusive_scan(xthread::cudaDefaultExecutionPolicy{},
      data2, data2+N, scan2, 
      [] __device__ (T a, T b){ return a+b; },
      [] __device__ (T a) { return a*10; },
      temp.data()
...
TEST_CASE("cudaflow.scan.int" * doctest::timeout(300)) {
  transform_scan<int, xthread::cudaFlow>();
}

TEST_CASE("cudaflow.scan.size_t" * doctest::timeout(300)) {
  transform_scan<size_t, xthread::cudaFlow>();
}

TEST_CASE("capture.transform_scan.int" * doctest::timeout(300)) {
  transform_scan<int, xthread::cudaFlowCapturer>();
}

TEST_CASE("capture.transform_scan.size_t" * doctest::timeout(300)) {
  transform_scan<size_t, xthread::cudaFlowCapturer>();
}
...
void merge_keys() {
  ...
  for(int N=0; N<=1234567; N = N*2 + 1) {
    ...
    auto a = xthread::cuda_malloc_shared<T>(N);
    auto b = xthread::cuda_malloc_shared<T>(N);
    auto c = xthread::cuda_malloc_shared<T>(2*N);
  
    xthread::cudaStream s;
    auto p = xthread::cudaDefaultExecutionPolicy{s};
    ...
    auto bufsz = xthread::cuda_merge_buffer_size<decltype(p)>(N, N);
    xthread::cudaDeviceVector<std::byte> buf(bufsz);

    xthread::cuda_merge(p, a, a+N, b, b+N, c, xthread::cuda_less<T>{}, buf.data());
    s.synchronize();
    ...
    taskflow.emplace([&](){
      F cudaflow;
      cudaflow.merge(a, a+N, b, b+N, c, xthread::cuda_less<T>{});
      run_and_wait(cudaflow);
    });
...
TEST_CASE("cudaflow.merge_keys.int" * doctest::timeout(300)) {
  merge_keys<int, xthread::cudaFlow>();
}

TEST_CASE("cudaflow.merge_keys.float" * doctest::timeout(300)) {
  merge_keys<float, xthread::cudaFlow>();
}
...
TEST_CASE("capture.merge_keys.float" * doctest::timeout(300)) {
  merge_keys<float, xthread::cudaFlowCapturer>();
}
...
void merge_keys_values() {
  ...
  for(int N=0; N<=1234567; N = N*2 + 1) {
    ...
    auto a_k = xthread::cuda_malloc_shared<T>(N);
    auto b_k = xthread::cuda_malloc_shared<T>(N);
    auto c_k = xthread::cuda_malloc_shared<T>(2*N);
    auto a_v = xthread::cuda_malloc_shared<int>(N);
    auto b_v = xthread::cuda_malloc_shared<int>(N);
    auto c_v = xthread::cuda_malloc_shared<int>(2*N);
    ...
    taskflow.emplace([&](){
      ...
      cudaflow.merge_by_key(
        a_k, a_k+N, a_v, b_k, b_k+N, b_v, c_k, c_v, xthread::cuda_less<T>{}
      );
      run_and_wait(cudaflow);
    });
...
void sort_keys() {
  ...
  for(int N=0; N<=1234567; N = N*2 + 1) {
    ...
    auto bufsz = xthread::cuda_sort_buffer_size<decltype(p), T>(N);
    xthread::cudaDeviceVector<std::byte> buf(bufsz);
    xthread::cuda_sort(p, a, a+N, xthread::cuda_less<T>{}, buf.data());
    s.synchronize();
    ...
    taskflow.emplace([&](){
      F cudaflow;
      cudaflow.sort(a, a+N, xthread::cuda_less<T>{});
      run_and_wait(cudaflow);
    });
...
void sort_keys_values() {
  ...
  for(int N=1; N<=1234567; N = N*2 + 1) {
    ...
    auto a = xthread::cuda_malloc_shared<T>(N);
    auto b = xthread::cuda_malloc_shared<int>(N);
    ...
    auto bufsz = xthread::cuda_sort_buffer_size<decltype(p), T, int>(N);
    xthread::cudaDeviceVector<std::byte> buf(bufsz);
    xthread::cuda_sort_by_key(p, a, a+N, b, xthread::cuda_less<T>{}, buf.data());
    s.synchronize();
    ...
    taskflow.emplace([&](){
      F cudaflow;
      cudaflow.sort_by_key(a, a+N, b, xthread::cuda_greater<T>{});
      run_and_wait(cudaflow);
    });
...
void find_if() {
  ...
  for(int N=0; N<=1234567; N += std::max(N/100, 1)) {
    ...
    auto a = xthread::cuda_malloc_shared<T>(N);
    auto r = xthread::cuda_malloc_shared<unsigned>(1);
    ...
    *r = 1234;
    ...
    xthread::cuda_find_if(p, a, a+N, r, []__device__(int v){ return v == 5000; });
    s.synchronize();
...
void min_element() {
  ...
  for(int N=0; N<=1234567; N += std::max(N/10, 1)) {
    ...
    *r = 1234;
    ...
    xthread::cudaDeviceVector<std::byte> buf(
      xthread::cuda_min_element_buffer_size<decltype(p), T>(N)
    );

    xthread::cuda_min_element(
      p, a, a+N, r, xthread::cuda_less<T>{}, buf.data()
    );
    s.synchronize();
    ...
    *r = 1234;
    ...
    taskflow.emplace([&](){
      F cudaflow;
      cudaflow.min_element(a, a+N, r, xthread::cuda_less<T>{});
      run_and_wait(cudaflow);
    });
...
void max_element() {
  ...
  for(int N=0; N<=1234567; N += std::max(N/10, 1)) {
    ...
    *r = 1234;
    ...
    xthread::cudaDeviceVector<std::byte> buf(
      xthread::cuda_max_element_buffer_size<decltype(p), T>(N)
    );

    xthread::cuda_max_element(p, a, a+N, r, xthread::cuda_less<T>{}, buf.data());
    s.synchronize();
    ...
    *r = 1234;
    ...
    taskflow.emplace([&](){
      F cudaflow;
      cudaflow.max_element(a, a+N, r, xthread::cuda_less<T>{});
      run_and_wait(cudaflow);
    });
  ...
  for(size_t rows = 1; rows <= 7999; rows*=2+3) {
    for(size_t cols = 1; cols <= 8021; cols*=3+5) {
      ...
      auto allocate = taskflow.emplace([&]() {
       ...
       //transpose
      auto cudaflow = taskflow.emplace([&](xthread::cudaFlow& cf) {
        auto h2d_input_t = cf.copy(dinput_mat, hinput_mat.data(), rows * cols).name("h2d");

        auto kernel_t = xthread::cudaBLAF(cf).transpose(
          dinput_mat,
          doutput_mat,
          rows,
          cols
  ...
  for(int m=1; m<=1992; m=2*m+1) {
    for(int k=1; k<=1012; k=2*k+3) {
      for(int n=1; n<=1998; n=2*n+8) {
        ...
        auto cuda = taskflow.emplace([&](xthread::cudaFlow& cf){
          auto pa = cf.copy(da, ha, m*k);
          auto pb = cf.copy(db, hb, k*n);

          auto op = xthread::cudaBLAF(cf).matmul(
            da, db, dc, m, k, n 
          ).name("op");
