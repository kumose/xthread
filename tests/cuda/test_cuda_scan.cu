...
void cuda_scan() {
  ...
  for(int n=0; n<=1234567; n = (n<=100) ? n+1 : n*2 + 1) {
    ...
    taskflow.emplace([n](){
      ...
      auto data1 = xthread::cuda_malloc_shared<int>(n);
      auto data2 = xthread::cuda_malloc_shared<int>(n);
      auto scan1 = xthread::cuda_malloc_shared<int>(n);
      auto scan2 = xthread::cuda_malloc_shared<int>(n);
      ...
      xthread::cuda_inclusive_scan(policy, data1, data1+n, scan1, xthread::cuda_plus<int>{}, buff);
      xthread::cuda_exclusive_scan(policy, data2, data2+n, scan2, xthread::cuda_plus<int>{}, buff);
...
void cuda_transform_scan() {
  ...
  for(int n=0; n<=1234567; n = (n<=100) ? n+1 : n*2 + 1) {
    ...
    taskflow.emplace([n](){
      ...
      xthread::cuda_transform_inclusive_scan(policy,
        data1, data1+n, scan1, xthread::cuda_plus<int>{},
        [] __device__ (int a) { return a*10; },
        buff
      );

      // transform exclusive scan
      xthread::cuda_transform_exclusive_scan(policy,
        data2, data2+n, scan2, xthread::cuda_plus<int>{},
