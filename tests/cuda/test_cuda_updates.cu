...
TEST_CASE("cudaGraph.Update.SingleTask") {
  ...
  auto var = xthread::cuda_malloc_shared<int>(1);
  *var = 1;
  REQUIRE(*var == 1);
...
TEST_CASE("cudaGraph.Update.Kernel") {
  ...
  auto vec = xthread::cuda_malloc_shared<int>(N);
...
TEST_CASE("cudaGraph.Update.Memcpy") {
  ...
  auto vec1 = xthread::cuda_malloc_shared<int>(N);
  auto vec2 = xthread::cuda_malloc_shared<int>(N);
  auto vec3 = xthread::cuda_malloc_shared<int>(N);
  ...
  xthread::cuda_free(vec1);
  xthread::cuda_free(vec2);
  xthread::cuda_free(vec3);
}
