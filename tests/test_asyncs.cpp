...
void async(unsigned W) {
  ...
  for(int i=0; i<N; ++i) {
    if(auto r = i%3; r==0) {
      ...
      fus.emplace_back(executor.async(xthread::DefaultTaskParams{}, [&](){
        counter.fetch_add(1, std::memory_order_relaxed);
        return -2;
      }));
    }
    else {
      fus.emplace_back(executor.async(xthread::TaskParams{std::to_string(i)}, [&](){
        counter.fetch_add(1, std::memory_order_relaxed);
        return -2;
      }));
    }
...
void mixed_executor_async(size_t N) {
  ...
  std::vector<xthread::Executor> executors(N);
  ...
  for(size_t j=0; j<T; j++) {
    for(size_t i=0; i<N; i++) {
      executors[i].async([&, i, j](){
        ...
        counter.fetch_add(1, std::memory_order_relaxed);
        auto n = j % N;
        executors[n].async([&, n](xthread::Runtime&){
          check_wid(n);
          counter.fetch_add(1, std::memory_order_relaxed);
        });
      });
      ...
      executors[i].silent_async([&, i, j](){
        ...
        counter.fetch_add(1, std::memory_order_relaxed);
        auto n = (j + 1) % N;
        executors[n].silent_async([&, n](xthread::Runtime){
          check_wid(n);
          counter.fetch_add(1, std::memory_order_relaxed);
        });
      });
...
void async_runtime(size_t W) {
  ...
  auto S1 = taskflow.emplace([&] (xthread::Runtime& sf){
  ...
  auto S2 = taskflow.emplace([&] (xthread::Runtime& sf){
  ...
  taskflow.emplace([&] (xthread::Runtime& sf){
    sf.silent_async([&](){ counter.fetch_add(1, std::memory_order_relaxed); });
    for(int i=0; i<1000; i++) {
      sf.async(
        [&](){ counter.fetch_add(1, std::memory_order_relaxed); }
...
void recursive_async_runtime(size_t num_threads) {
  ...
  // async
  counter = 0;
  executor.async([&](xthread::Runtime& rt1){
    counter.fetch_add(1, std::memory_order_relaxed);
    for(size_t i=0; i<N1; ++i) {
      rt1.silent_async([&](xthread::Runtime& rt2) {
        counter.fetch_add(1, std::memory_order_relaxed);
        for(size_t j=0; j<N2; ++j) {
          rt2.silent_async([&](xthread::Runtime&) {
            counter.fetch_add(1, std::memory_order_relaxed);
          });
        } 
      });
  ...
  auto [task, fu] = executor.dependent_async([&](xthread::Runtime& rt1){
    counter.fetch_add(1, std::memory_order_relaxed);
    for(size_t i=0; i<N1; ++i) {
      rt1.silent_async([&](xthread::Runtime& rt2) {
        counter.fetch_add(1, std::memory_order_relaxed);
