  ...
  for(int beg=10; beg>=-10; --beg) {
    for(int end=beg; end>=-10; --end) {
      for(int s=1; s<=beg-end; ++s) {
        ...
        size_t n = xthread::distance(beg, end, -s);

        for(size_t c=0; c<10; c++) {
          taskflow.clear();
          counter = 0;

          xthread::IndexRange range(beg, end, -s);
          REQUIRE(range.size() == n);

          taskflow.for_each_by_index(range, [&] (xthread::IndexRange<int> lrange) {
            size_t l = 0;
    ...
    for(int end=beg; end>=-10; --end) {
      for(int s=1; s<=beg-end; ++s) {
        ...
        for(size_t c=0; c<10; c++) {
          ...
          xthread::IndexRange range(0, 0, 0);
          ...
          auto loop_range = taskflow.for_each_by_index(std::ref(range), [&] (xthread::IndexRange<int> lrange) {
            size_t l = 0;
            for(auto j=lrange.begin(); j>lrange.end(); j+=lrange.step_size()) {
              l++;
            }
...
TEST_CASE("ClosureWrapper.for_each_index.Static" * doctest::timeout(300))
{
  for (int tc = 1; tc < 64; tc++)
  {
    ...
    taskflow.for_each_index(0, 100, 1, 
      ...
      xthread::StaticPartitioner(1, [&](auto &&task){
...
TEST_CASE("ClosureWrapper.for_each_index.Dynamic" * doctest::timeout(300))
{
  for (int tc = 1; tc < 64; tc++)
  {
    ...
    taskflow.for_each_index(0, UPPER, 1, 
      ...
      xthread::DynamicPartitioner(UPPER/tc/2, [&](auto&& task){
        wrapper_called_count++;
        GetThreadSpecificContext() = tc;
        task();
        GetThreadSpecificContext() = 0;
...
TEST_CASE("ClosureWrapper.for_each.Dynamic" * doctest::timeout(300))
{
  for (int tc = 1; tc < 16; tc++)
  {
    ...
    taskflow.for_each(range.begin(), range.end(), 
      ...
      xthread::DynamicPartitioner(1, [&](auto&& task){
        wrapper_called_count++;
        GetThreadSpecificContext() = tc;
        task();
        GetThreadSpecificContext() = 0;
...
TEST_CASE("MultipleParallelForEach.Static.1thread") {
  multiple_for_each<xthread::StaticPartitioner<>>(1);
}

TEST_CASE("MultipleParallelForEach.Static.2threads") {
  multiple_for_each<xthread::StaticPartitioner<>>(2);
}

TEST_CASE("MultipleParallelForEach.Static.3threads") {
  multiple_for_each<xthread::StaticPartitioner<>>(3);
}

TEST_CASE("MultipleParallelForEach.Static.4threads") {
  multiple_for_each<xthread::StaticPartitioner<>>(4);
}

TEST_CASE("MultipleParallelForEach.Dynamic.1thread") {
  multiple_for_each<xthread::DynamicPartitioner<>>(1);
}

TEST_CASE("MultipleParallelForEach.Dynamic.2threads") {
  multiple_for_each<xthread::DynamicPartitioner<>>(2);
}

TEST_CASE("MultipleParallelForEach.Dynamic.3threads") {
  multiple_for_each<xthread::DynamicPartitioner<>>(3);
}

TEST_CASE("MultipleParallelForEach.Dynamic.4threads") {
  multiple_for_each<xthread::DynamicPartitioner<>>(4);
}

TEST_CASE("MultipleParallelForEach.Guided.1thread") {
  multiple_for_each<xthread::GuidedPartitioner<>>(1);
}

TEST_CASE("MultipleParallelForEach.Guided.2threads") {
  multiple_for_each<xthread::GuidedPartitioner<>>(2);
}

TEST_CASE("MultipleParallelForEach.Guided.3threads") {
  multiple_for_each<xthread::GuidedPartitioner<>>(3);
}

TEST_CASE("MultipleParallelForEach.Guided.4threads") {
  multiple_for_each<xthread::GuidedPartitioner<>>(4);
}
...
void async(unsigned W) {
  ...
  for(size_t N=0; N<=65536; N =((N == 0) ? 1 : N << 1)) {
    ...
    executor.async(xthread::make_for_each_task(
      data.begin(), data.begin() + N/2, [](int& d){ d = -10; }
    )); 
    
    executor.async(xthread::make_for_each_index_task(
...
void silent_async(unsigned W) {
  ...
  // initialize data to 10 and -10
  executor.silent_async(xthread::make_for_each_task(
    data.begin(), data.begin() + N/2, [](int& d){ d = 10; }
  )); 
  
  executor.silent_async(xthread::make_for_each_index_task(
    N/2, N, size_t{1}, [&] (size_t i) { data[i] = -10; }
  ));
...
void dependent_async(unsigned W) {
  ...
  for(size_t N=0; N<=65536; N =((N == 0) ? 1 : N << 1)) {
    ...
    // initialize data to -10 and 10
    executor.dependent_async(xthread::make_for_each_task(
      data.begin(), data.begin() + N/2, [](int& d){ d = -10; }
    )); 
    
    executor.dependent_async(xthread::make_for_each_index_task(
      N/2, N, size_t{1}, [&] (size_t i) { data[i] = 10; }
    ));
...
void silent_dependent_async(unsigned W) {
  ...
  // initialize data to 10 and -10
  executor.silent_dependent_async(xthread::make_for_each_task(
    data.begin(), data.begin() + N/2, [](int& d){ d = 10; }
  )); 
  
  executor.silent_dependent_async(xthread::make_for_each_index_task(
    N/2, N, size_t{1}, [&] (size_t i) { data[i] = -10; }
  ));
...
void nested_for_loop(unsigned W) {
  ...
  executor.async(xthread::make_for_each_index_task(0, N1, 1, [&](int i){
    executor.async(xthread::make_for_each_index_task(0, N2, 1, [&, i](int j) {
      data[i][j] = i + j;
    }));
  })); 
