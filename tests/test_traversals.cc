...
void dynamic_traversal(unsigned W) {
  ...
  std::function<void(Node*, xthread::Subflow&)> traverse;

  traverse = [&] (Node* n, xthread::Subflow& subflow) {
    REQUIRE(!n->visited);
    n->visited = true;
    ...
    for(size_t i=0; i<S; i++) {
      if(n->successors[i]->dependents.fetch_sub(1) == 1) {
        ...
        subflow.emplace([s=n->successors[i], &traverse](xthread::Subflow &subflow2){
          traverse(s, subflow2);
        });
      }
    }
  ...
  for(size_t i=0; i<src.size(); i++) {
    tf.emplace([s=src[i], &traverse](xthread::Subflow& subflow){
      traverse(s, subflow);
    });
  }
    ...
    threads.emplace_back([&](){
      ...
      traverse = [&] (Node* n, xthread::Subflow& subflow) {
        ...
        for(size_t i=0; i<S; i++) {
          if(n->successors[i]->dependents.fetch_sub(1) == 1) {
            n->successors[i]->level = ++level;
            subflow.emplace([s=n->successors[i], &traverse](xthread::Subflow &subflow){
              traverse(s, subflow);
            });
          }
        }
