...
// --------------------------------------------------------
void pipeline_1P(size_t L, unsigned w, xthread::PipeType type) {
  ...
  for (size_t N = 0; N <= maxN; N++) {
    ...
    if (type == xthread::PipeType::SERIAL) {
      ...
      size_t j = 0;
      xthread::Pipeline pl (L, xthread::Pipe{type, [L, N, &j, &source](auto& pf) mutable {
        if (j == N) {
          pf.stop();
          return;
        }
    ...
    // parallel pipe
    //else if(type == xthread::PipeType::PARALLEL) {
    //
    //  xthread::Taskflow taskflow;
    ...
    //  xthread::Pipeline pl(L, xthread::Pipe{type,
...
void pipeline_2P_SS(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    xthread::Pipeline pl(
      L,
      xthread::Pipe{xthread::PipeType::SERIAL, [N, &source, &j1, &mybuffer, L](auto& pf) mutable {
        if(j1 == N) {
          pf.stop();
          return;
        }
      ...
      xthread::Pipe{xthread::PipeType::SERIAL, [N, &source, &j2, &mybuffer, L](auto& pf) mutable {
        REQUIRE(j2 < N);
        REQUIRE(pf.token() % L == pf.line());
...
void pipeline_2P_SP(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    xthread::Pipeline pl(L,
      ...
      xthread::Pipe{xthread::PipeType::PARALLEL,
...
void pipeline_3P_SSS(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    xthread::Pipeline pl(L,
      ...
      xthread::Pipe{xthread::PipeType::SERIAL, [N, &source, &j3, &mybuffer, L](auto& pf) mutable {
        REQUIRE(j3 < N);
...
void pipeline_3P_SSP(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    xthread::Pipeline pl(L,
      ...
      xthread::Pipe{xthread::PipeType::PARALLEL, [N, &j3, &mutex, &collection, &mybuffer, L](auto& pf) mutable {
        REQUIRE(j3++ < N);
        {
          std::scoped_lock<std::mutex> lock(mutex);
          REQUIRE(pf.token() % L == pf.line());
...
void pipeline_3P_SPS(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    xthread::Pipeline pl(L,
      ...
      xthread::Pipe{xthread::PipeType::PARALLEL, [N, &j2, &mutex, &collection, &mybuffer, L](auto& pf) mutable {
        REQUIRE(j2++ < N);
...
void pipeline_3P_SPP(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    xthread::Pipeline pl(L,
      ...
      xthread::Pipe{xthread::PipeType::PARALLEL, [N, &j2, &mutex2, &collection2, &mybuffer, L](auto& pf) mutable {
        REQUIRE(j2++ < N);
        //*pf.output() = *pf.input() + 1;
        {
      ...
      xthread::Pipe{xthread::PipeType::PARALLEL, [N, &j3, &mutex3, &collection3, &mybuffer, L](auto& pf) mutable {
        REQUIRE(j3++ < N);
        {
...
void three_parallel_pipelines(size_t L, unsigned w) {
  ...
  for(size_t N = 0; N <= maxN; N++) {
    ...
    // pipeline 1 is SSSS
    xthread::Pipeline pl1(L,
      xthread::Pipe{xthread::PipeType::SERIAL, [N, &source, &j1_1, &mybuffer1, L](auto& pf) mutable {
        if(j1_1 == N) {
          pf.stop();
          return;
        }
      ...
      xthread::Pipe{xthread::PipeType::SERIAL, [N, &source, &j1_2, &mybuffer1, L](auto& pf) mutable {
        REQUIRE(j1_2 < N);
        REQUIRE(pf.token() % L == pf.line());
      ...
      xthread::Pipe{xthread::PipeType::SERIAL, [N, &source, &j1_3, &mybuffer1, L](auto& pf) mutable {
        REQUIRE(j1_3 < N);
        REQUIRE(pf.token() % L == pf.line());
      ...
      xthread::Pipe{xthread::PipeType::SERIAL, [N, &source, &j1_4, &mybuffer1, L](auto& pf) mutable {
        REQUIRE(j1_4 < N);
        REQUIRE(pf.token() % L == pf.line());
    ...
    // pipeline 2 is SSP
    xthread::Pipeline pl2(L,
      xthread::Pipe{xthread::PipeType::SERIAL, [N, &source, &j2_1, &mybuffer2, L](auto& pf) mutable {
        if(j2_1 == N) {
          pf.stop();
          return;
        }
      ...
      xthread::Pipe{xthread::PipeType::SERIAL, [N, &source, &j2_2, &mybuffer2, L](auto& pf) mutable {
        REQUIRE(j2_2 < N);
      ...
      xthread::Pipe{xthread::PipeType::PARALLEL, [N, &j2_3, &mutex2_3, &collection2_3, &mybuffer2, L](auto& pf) mutable {
        REQUIRE(j2_3++ < N);
        {
    ...
    // pipeline 3 is SP
    xthread::Pipeline pl3(L,
      xthread::Pipe{xthread::PipeType::SERIAL, [N, &source, &j3_1, &mybuffer3, L](auto& pf) mutable {
        if(j3_1 == N) {
          pf.stop();
          return;
        }
...
void looping_pipelines(size_t L, unsigned w) {
  ...
  xthread::Pipeline pl(L,
    ...
    xthread::Pipe{xthread::PipeType::PARALLEL, [&N, &j4, &mutex4, &collection4, &mybuffer, L](auto& pf) mutable {
      REQUIRE(j4++ < N);
      {
...
void ifelse_pipeline(size_t L, unsigned w) {
  ...
  for(size_t N = 1; N < maxN; ++N) {
    ...
    xthread::Pipeline pl(L,
      // pipe 1
      xthread::Pipe(xthread::PipeType::SERIAL, [&, N](auto& pf){
        if(pf.token() == N) {
          pf.stop();
          return;
        }
      ...
      // pipe 2
      xthread::Pipe(xthread::PipeType::PARALLEL, [&](auto& pf){
      ...
      // pipe 3
      xthread::Pipe(xthread::PipeType::SERIAL, [&](auto& pf){
  ...
  for (size_t N = 1; N < maxN; ++N) {
    for(size_t subN = 1; subN < maxsubN; ++subN) {
      ...
      xthread::Pipeline pl(L,
        ...
        // begin of pipe 1 -----------------------------
        xthread::Pipe{xthread::PipeType::SERIAL, [&, N, subN, subL](auto& pf) mutable {
          if(j1 == N) {
            pf.stop();
            return;
          }
          ...
          // subpipeline
          xthread::Pipeline subpl(subL,

            // subpipe 1
            xthread::Pipe{xthread::PipeType::SERIAL, [&, subN](auto& subpf) mutable {
              if(subj1 == subN) {
                subpf.stop();
                return;
              }
            ...
            // subpipe 2
            xthread::Pipe{xthread::PipeType::PARALLEL, [&, subN](auto& subpf) mutable {
              REQUIRE(subj2++ < subN);
              REQUIRE(subpf.token() % subL == subpf.line());
        ...
        // end of pipe 1 -----------------------------
         ...
         //begin of pipe 2 ---------------------------
        xthread::Pipe{xthread::PipeType::PARALLEL, [&, N, subN, subL](auto& pf) mutable {
        ...
        // begin of pipe 4 ---------------------------
        xthread::Pipe{xthread::PipeType::SERIAL, [&, subN](auto& pf) mutable {
