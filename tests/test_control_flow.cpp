...
TEST_CASE("Cond.Types" * doctest::timeout(300)) {
  ...
  static_assert(xthread::is_condition_task_v<decltype(explicit_c)>);
  static_assert(xthread::is_condition_task_v<decltype(implicit_c)>);

  REQUIRE(explicit_task.type() == xthread::TaskType::CONDITION);
  REQUIRE(implicit_task.type() == xthread::TaskType::CONDITION);
}
...
void conditional_spawn(
  ...
  const int max_depth,
  int depth,
  xthread::Subflow& subflow
)  {
  if(depth < max_depth) {
    for(int i=0; i<2; i++) {
      auto A = subflow.emplace([&](){ counter++; });
      auto B = subflow.emplace(
        [&, max_depth, depth=depth+1](xthread::Subflow& sf2){
...
TEST_CASE("BTreeCondition" * doctest::timeout(300)) {
  for(unsigned w=1; w<=8; ++w) {
    for(int l=1; l<12; l++) {
      xthread::Taskflow flow;
      std::vector<xthread::Task> prev_tasks;
...
TEST_CASE("DynamicBTreeCondition" * doctest::timeout(300)) {
  for(unsigned w=1; w<=8; ++w) {
    ...
    xthread::Taskflow flow;
    flow.emplace([&](xthread::Subflow& subflow) {
      counter++;
...
void nested_cond(unsigned w) {
  ...
  xthread::Taskflow flow;
  auto S = flow.emplace([](){});
  auto A = flow.emplace([&] (xthread::Subflow& subflow) mutable {
    //           ___________
    //          |           |
    //          v           |
    //   S1 -> A1 -> B1 -> cond
    ...
    auto B1 = subflow.emplace([&](xthread::Subflow& sf){
...
void hierarchical_condition(unsigned w) {
  ...
  xthread::Executor executor(w);
  xthread::Taskflow tf0("c0");
  xthread::Taskflow tf1("c1");
  xthread::Taskflow tf2("c2");
  xthread::Taskflow tf3("top");
  ...
  auto sbf1 = tf3.emplace([&](xthread::Subflow& sbf){
...
void condition_subflow(unsigned W) {
  ...
  auto subflow = taskflow.emplace([&](xthread::Subflow& sf){
    sf.emplace([&, i](){
      REQUIRE(i<I);
      data[i] = i*(i+1)/2*123;;
...
TEST_CASE("MultiCond.Types" * doctest::timeout(300)) {
  ...
  auto explicit_mc = [](){
    xthread::SmallVector<int> v;
    return v;
  };

  auto implicit_mc = []() -> xthread::SmallVector<int> {
    return {1, 2, 3, 9};
  };
  ...
  static_assert(xthread::is_multi_condition_task_v<decltype(explicit_mc)>);
  static_assert(xthread::is_multi_condition_task_v<decltype(implicit_mc)>);
...
void multiple_loops(unsigned W) {
  ...
  auto B = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> xthread::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto C = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> xthread::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto D = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> xthread::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto E = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> xthread::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
...
void binary_tree(unsigned w) {
  ...
  for(int i=1; i<(1<<N); i++) {
    tasks.emplace_back(taskflow.emplace([&counter]() -> xthread::SmallVector<int>{
      counter.fetch_add(1, std::memory_order_relaxed);
      return {0, 1};
    }));
  }
...
void multi_cond_multiple_runs(unsigned W) {
  ...
  xthread::Task tempTask1 = taskflow.emplace([]() {});
  xthread::Task tempTask2 = taskflow.emplace([]() {});
