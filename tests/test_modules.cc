...
void module4(unsigned W) {
  ...
  C.precede(D);
  D.precede(E);
  auto m1_1 = f1.emplace(xthread::make_module_task(f0));
  E.precede(m1_1);
  ...
  auto m1_2 = f1.emplace(xthread::make_module_task(f0));
  m1_1.precede(m1_2);
...
void parallel_modules(unsigned W) {
  ...
  std::vector<xthread::Taskflow> taskflows(100);
...
void module_with_async_launch(unsigned W) {
  ...
  executor.async(xthread::make_module_task(f1)).get();
...
void module_with_silent_async_launch(unsigned W) {
  ...
  executor.silent_async(xthread::make_module_task(f1));
  executor.wait_for_all();
...
void module_with_dependent_async_launch(unsigned W) {
  ...
  auto [task, future] = executor.dependent_async(xthread::make_module_task(f1));
...
void module_with_silent_dependent_async_launch(unsigned W) {
  ...
  auto task = executor.silent_dependent_async(xthread::make_module_task(f1));
