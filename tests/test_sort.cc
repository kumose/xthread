...
void async(size_t W) {
  ...
  for(size_t n=0; n < 100000; n = (n ? n*10 : 1)) {
    ...
    executor.async(xthread::make_sort_task(data.begin(), data.end()));
...
void dependent_async(size_t W) {
  ...
  for(size_t n=0; n < 100000; n = (n ? n*10 : 1)) {
    ...
    executor.dependent_async(xthread::make_sort_task(data.begin(), data.end()));
    executor.wait_for_all();
    REQUIRE(std::is_sorted(data.begin(), data.end()));
  }
}
...
void silent_async(size_t W) {
  ...
  for(size_t n=0; n < 100000; n = (n ? n*10 : 1)) {
    ...
    executor.silent_async(xthread::make_sort_task(data.begin(), data.end()));
    executor.wait_for_all();
    REQUIRE(std::is_sorted(data.begin(), data.end()));
  }
}
...
void silent_dependent_async(size_t W) {
  ...
  for(size_t n=0; n < 100000; n = (n ? n*10 : 1)) {
    ...
    executor.silent_dependent_async(xthread::make_sort_task(data.begin(), data.end()));
    executor.wait_for_all();
    REQUIRE(std::is_sorted(data.begin(), data.end()));
  }
}
...
void bubble_sort(unsigned W) {
  ...
  for(int end=1; end <= 1000; end *= 10) {
    ...
    auto even_phase = taskflow.emplace([&](xthread::Subflow& sf){
    ...
    auto odd_phase = taskflow.emplace([&](xthread::Subflow& sf) {
...
void selection_sort(unsigned W) {
  ...
  std::function<
    void(xthread::Subflow& sf, std::vector<int>&, int, int, int&)
  > spawn;
  ...
  spawn = [&] (
    ...
    auto SL = sf.emplace(
      [&spawn, &data, beg, m, l=minl] (xthread::Subflow& sf2) mutable {
    ...
    auto SR = sf.emplace(
      [&spawn, &data, m, end, r=minr] (xthread::Subflow& sf2) mutable {
  ...
  for(int end=1; end <= 256; end <<= 1) {
    ...
    auto argmin = taskflow.emplace(
      [&spawn, &data, &beg, end, &min](xthread::Subflow& sf) mutable {
...
void merge_sort(unsigned W) {
  ...
  std::function<void(xthread::Subflow& sf, std::vector<int>&, int, int)> spawn;

  spawn = [&] (xthread::Subflow& sf, std::vector<int>& data, int beg, int end) mutable {
    ...
    auto SL = sf.emplace([&spawn, &data, beg, m] (xthread::Subflow& sf2) {
    ...
    auto SR = sf.emplace([&spawn, &data, m, end] (xthread::Subflow& sf2) {
  ...
  for(int end=10; end <= 10000; end *= 10) {
    ...
    taskflow.emplace([&spawn, &data, end](xthread::Subflow& sf){
      spawn(sf, data, 0, end);
    }).name(std::string("[0")
          + ":"
          + std::to_string(end) + ")");
...
void quick_sort(unsigned W) {
  ...
  std::function<void(xthread::Subflow& sf, std::vector<int>&, itr_t, itr_t)> spawn;
  ...
  spawn = [&] (
    ...
    sf.emplace([&spawn, &data, beg, pvt] (xthread::Subflow& sf2) {
    ...
    sf.emplace([&spawn, &data, pvt, end] (xthread::Subflow& sf2) {
  ...
  for(size_t end=1; end <= 10000; end *= 10) {
    ...
    taskflow.emplace([&spawn, &data](xthread::Subflow& sf){
