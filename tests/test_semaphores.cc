...
void overlapped_semaphores(size_t W) {
  ...
  xthread::Executor executor(W);
  xthread::Taskflow taskflow;
  xthread::Semaphore semaphore1(1);
  xthread::Semaphore semaphore4(4);
...
void conflict_graph(size_t W) {
  ...
  xthread::Executor executor(W);
  xthread::Taskflow taskflow;
  xthread::Semaphore conflict_AB(1);
  xthread::Semaphore conflict_AC(1);

  int counter {0};
  std::mutex mutex;

  xthread::Task A = taskflow.emplace([&](){ counter++; });
...
void semaphores_in_module(unsigned W) {
  ...
  xthread::Taskflow taskflow2;
  xthread::Executor executor(W);
  std::vector<xthread::Semaphore> semaphores(10);
...
void linear_chain(unsigned W) {
  ...
  std::vector<xthread::CachelineAligned<size_t>> counters(L);
  std::vector<xthread::Semaphore> semaphores(L);
...
void deadlock(unsigned W) {
  ...
  xthread::Executor exec(W);
  xthread::Taskflow flow;
  xthread::Semaphore semaphore(8);

  for(size_t i = 0; i < 5; ++i) {

    xthread::Task commonTask = flow.emplace([i]() {
      std::string str;
    ...
    for(size_t j = 0; j < 8; ++j) {
      xthread::Task preTask = flow.emplace([i, j]() {
        std::string str;
      ...
      xthread::Task postTask = flow.emplace([i, j]() {
        std::string str;
