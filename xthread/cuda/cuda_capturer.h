...
#include <xthread/cuda/cuda_optimizer.h>

/**
@file cuda_capturer.h
...
namespace xthread {
    ...
    The usage of xthread::cudaFlowCapturer is similar to xthread::cudaFlow, except users can
    call the method xthread::cudaFlowCapturer::on to capture a sequence of asynchronous
    ...
    @code{.cpp}
    taskflow.emplace([](xthread::cudaFlowCapturer& capturer){
    ...
    Similar to xthread::cudaFlow, a %cudaFlowCapturer is a task (xthread::Task)
    created from xthread::Taskflow
    and will be run by @em one worker thread in the executor.
    That is, the callable that describes a %cudaFlowCapturer
    will be executed sequentially.
    Inside a %cudaFlow capturer task, different GPU tasks (xthread::cudaTask) may run
    in parallel depending on the selected optimization algorithm.
    By default, we use xthread::cudaFlowRoundRobinOptimizer to transform a user-level
    ...
    */
    ...
    public:
        ...
        can be run by the caller thread using xthread::cudaFlowCapturer::run.
        */
        ...
        */
        ...
        @return a xthread::cudaTask handle
        ...
        */
        ...
        The method is similar to xthread::cudaFlowCapturer::noop but
        ...
        */
        ...
        a user-described %cudaFlow:
          + xthread::cudaFlowSequentialOptimizer
          + xthread::cudaFlowRoundRobinOptimizer
          + xthread::cudaFlowLinearOptimizer

        By default, xthread::cudaFlowCapturer uses the round-robin optimization
