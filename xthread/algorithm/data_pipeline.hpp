...
namespace xthread {
    ...
    (specified by xthread::PipeType) and is associated with a callable to invoke
    by the pipeline scheduler.

    You need to use the template function, xthread::make_data_pipe, to create
    a data pipe. The input and output types of a xthread::DataPipe should be decayed types
    ...
    @code{.cpp}
    xthread::make_data_pipe<int, std::string>(
    ...
    of xthread::Pipeflow in the second argument to probe the runtime information
    for a stage task, such as its line number and token number:

    @code{.cpp}
    xthread::make_data_pipe<int, std::string>(
      xthread::PipeType::SERIAL,
      [](int& input, xthread::Pipeflow& pf) {
    ...
    */
    ...
    public:
        ...
        */
        ...
        You should use the helper function, xthread::make_data_pipe,
        to create a DataPipe object, especially when you need xthread::DataPipe
        ...
        */
        ...
        A data pipe can be either parallel (xthread::PipeType::PARALLEL) or serial
        (xthread::PipeType::SERIAL).
        */
        PipeType type() const {
            return _type;
    ...
    /**
    @brief function to construct a data pipe (xthread::DataPipe)

    @tparam Input input data type
    @tparam Output output data type
    @tparam C callable type

    xthread::make_data_pipe is a helper function to create a data pipe (xthread::DataPipe)
    in a data-parallel pipeline (xthread::DataPipeline).
    The first argument specifies the direction of the data pipe,
    either xthread::PipeType::SERIAL or xthread::PipeType::PARALLEL,
    ...
    The callable can additionally take a reference of xthread::Pipeflow,
    ...
    */
    ...
    Similar to xthread::Pipeline, a xthread::DataPipeline is a composable graph object
    for users to create a <i>data-parallel pipeline scheduling framework</i>
    using a module task in a taskflow.
    The only difference is that xthread::DataPipeline provides a data abstraction
    ...
    int main() {
      ...
      xthread::DataPipeline pl(num_lines,
        xthread::make_data_pipe<void, int>(xthread::PipeType::SERIAL, [&](xthread::Pipeflow& pf) -> int{
          if(pf.token() == 5) {
            pf.stop();
            return 0;
          }
        ...
        xthread::make_data_pipe<int, std::string>(xthread::PipeType::SERIAL, [](int& input) {
          return std::to_string(input + 100);
        }),
        xthread::make_data_pipe<std::string, void>(xthread::PipeType::SERIAL, [](std::string& input) {
    ...
    */
    ...
    public:
        ...
        */
        ...
        The first pipe must define a serial direction (xthread::PipeType::SERIAL)
    ...
    void DataPipeline<Ps...>::_on_pipe(Pipeflow &pf, NonpreemptiveRuntime &) {
        visit_tuple([&](auto &&pipe) {
            ...
            if constexpr (std::is_invocable_v<callable_t, Pipeflow &>) {
                // [](xthread::Pipeflow&) -> void {}, i.e., we only have one pipe
                if constexpr (std::is_void_v<output_t>) {
                    pipe._callable(pf);
                    // [](xthread::Pipeflow&) -> output_t {}
                } else {
            ...
            // other pipes with pipeflow in the second argument
                ...
                // [](input_t&, xthread::Pipeflow&) -> void {}
                if constexpr (std::is_void_v<output_t>) {
                    pipe._callable(std::get<input_t>(_buffer[pf._line].data), pf);
                    // [](input_t&, xthread::Pipeflow&) -> output_t {}
                } else {
                    _buffer[pf._line].data = pipe._callable(
    ...
    void DataPipeline<Ps...>::_build() {
        ...
        // line task
        for (size_t l = 0; l < num_lines(); l++) {
            _tasks[l + 1] = fb.emplace([this, l](xthread::NonpreemptiveRuntime &rt) mutable {
