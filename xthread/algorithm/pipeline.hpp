...
namespace xthread {
    ...
    @code{.cpp}
    xthread::Pipe{xthread::PipeType::SERIAL, [](xthread::Pipeflow& pf){
    ...
    Pipeflow can only be created privately by the xthread::Pipeline and
    ...
    */
    ...
    @em parallel direction or @em serial direction (specified by xthread::PipeType)
    and is coupled with a callable to invoke by the pipeline scheduler.
    The callable must take a referenced xthread::Pipeflow object in the first argument:

    @code{.cpp}
    Pipe{PipeType::SERIAL, [](xthread::Pipeflow&){}}
    @endcode
    ...
    */
    template<typename C = std::function<void(xthread::Pipeflow &)> >
    class Pipe {
        template<typename... Ps>
        friend class Pipeline;
    ...
    public:
        ...
        */
        ...
        @param d pipe type (xthread::PipeType)
        @param callable callable type

        The constructor constructs a pipe with the given direction
        (xthread::PipeType::SERIAL or xthread::PipeType::PARALLEL) and the given callable.
        The callable must take a referenced xthread::Pipeflow object in the first argument.
        ...
        */
        ...
        @param type a xthread::PipeType variable
        */
        ...
        @param callable a callable object constructible from std::function<void(xthread::Pipeflow&)>
    ...
    xthread::Pipeline pipeline(num_lines,
    ...
    xthread::Task task = taskflow.composed_of(pipeline)
                            .name("pipeline");
    ...
    Internally, xthread::Pipeline uses std::tuple to store the given sequence of pipes.
    ...
    If applications need to change these pipes, please use xthread::ScalablePipeline.
    */
    template<typename... Ps>
    class Pipeline {
    ...
    <i>pipeline scheduling framework</i> using a module task in a taskflow.
    Unlike xthread::Pipeline that instantiates all pipes upon the construction time,
    xthread::ScalablePipeline allows variable assignments of pipes using range iterators.
    ...
    auto pipe_callable = [&buffer] (xthread::Pipeflow& pf) mutable {
      switch(pf.pipe()) {
    ...
    for(size_t i=0; i<3; i++) {
      pipes.emplace_back(xthread::PipeType::SERIAL, pipe_callable);
    }

    // create a pipeline of four parallel lines based on the given vector of pipes
    xthread::ScalablePipeline pl(num_lines, pipes.begin(), pipes.end());
    ...
    Each pipe has the same type of `%xthread::Pipe<%std::function<void(%xthread::Pipeflow&)>>`
