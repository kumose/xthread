...
namespace xthread {
    ...
    */
    ...
    auto Algorithm::make_module_task(T &&target) {
        return [&target=std::forward<T>(target)](xthread::Runtime &rt) {
    ...
    @tparam T type of the graph object, which must define the method `xthread::Graph& graph()`
    ...
    The module task maker, xthread::make_module_task, is basically the same as xthread::Taskflow::composed_of
    ...
    xthread::Task m1 = taskflow1.composed_of(taskflow2);

    // approach 2: composition using make_module_task
    xthread::Task m1 = taskflow1.emplace(xthread::make_module_task(taskflow2));
    @endcode
