...
#include <xthread/dsl/meta_macro.hpp>
#include <xthread/dsl/task_analyzer.hpp>
#include <xthread/dsl/task_trait.hpp>
...
#define TF_CONTEXT_1(name) xthread::dsl::EmptyContext
...
#define make_task(name, ...)                                                    \
  struct TF_GET_FIRST name : xthread::dsl::TaskSignature,                           \
...
#define some_tasks(...) auto (*)(xthread::dsl::SomeTask<__VA_ARGS__>)
// same as some_tasks
...
#define build_taskflow(...) xthread::dsl::taskDsl<void TF_MAP(TF_CHAIN, __VA_ARGS__)>
