...
#include <xthread/core/graph.hpp>
...
namespace xthread {
    ...
    */
    template<typename C>
    struct is_subflow_task<C, std::enable_if_t<std::is_invocable_v<C, xthread::Subflow &> > >
            : std::is_same<std::invoke_result_t<C, xthread::Subflow &>, void> {
    };
    ...
    */
    template<typename C>
    struct is_runtime_task<C, std::enable_if_t<std::is_invocable_v<C, xthread::Runtime &> > >
            : std::is_same<std::invoke_result_t<C, xthread::Runtime &>, void> {
    };
    ...
    */
    template<typename C>
    struct is_runtime_task<C, std::enable_if_t<std::is_invocable_v<C, xthread::NonpreemptiveRuntime &> > >
            : std::is_same<std::invoke_result_t<C, xthread::NonpreemptiveRuntime &>, void> {
    };
    ...
    */
    ...
    std::function<xthread::SmallVector<int>()>.
    */
    template<typename C>
    ...
    A task created from a taskflow can be one of the following types:
      + xthread::TaskType::STATIC - @ref StaticTasking
      + xthread::TaskType::CONDITION - @ref ConditionalTasking
      + xthread::TaskType::RUNTIME - @ref RuntimeTasking
      + xthread::TaskType::SUBFLOW - @ref SubflowTasking
      + xthread::TaskType::MODULE - @ref ComposableTasking

    @code{.cpp}
    xthread::Task task1 = taskflow.emplace([](){}).name("static task");
    xthread::Task task2 = taskflow.emplace([](){ return 3; }).name("condition task");
    xthread::Task task3 = taskflow.emplace([](xthread::Runtime&){}).name("runtime task");
    xthread::Task task4 = taskflow.emplace([](xthread::Subflow& sf){
      xthread::Task stask1 = sf.emplace([](){});
      xthread::Task stask2 = sf.emplace([](){});
    }).name("subflow task");
    xthread::Task task5 = taskflow.composed_of(taskflow2).name("module task");
    @endcode

    A xthread::Task is polymorphic.
    Once created, you can assign a different task type to it using xthread::Task::work.
    For example, the code below creates a static task and then reworks it to a subflow task:

    @code{.cpp}
    xthread::Task task = taskflow.emplace([](){}).name("static task");
    task.work([](xthread::Subflow& sf){
      xthread::Task stask1 = sf.emplace([](){});
      xthread::Task stask2 = sf.emplace([](){});
    }).name("subflow task");
    @endcode
    ...
    @attention
    xthread::Task does not own the lifetime of the associated node.
    ...
    */
    ...
    public:
        ...
        */
        ...
        xthread::Task A = taskflow.emplace([](){ std::cout << "Task A\n"; });
        xthread::Task B(A);
        ...
        */
        ...
        @code{.cpp}
        xthread::Task A = taskflow.emplace([](){ std::cout << "A\n"; });
        xthread::Task B;
        ...
        */
        ...
        @code{.cpp}
        xthread::Task A = taskflow.emplace([](){ std::cout << "A\n"; });
        xthread::Task B = taskflow.emplace([](){ std::cout << "B\n"; });
        ...
        */
        ...
        @code{.cpp}
        xthread::Task task = taskflow.emplace([](){});
        task.name("MyTask");
        ...
        */
        ...
        @code{.cpp}
        xthread::Task A = taskflow.emplace([](){});
        xthread::Task B = taskflow.emplace([](){});
        ...
        */
        ...
        @code{.cpp}
        xthread::Task task = taskflow.emplace([](){}).name("foo");
        ...
        */
        ...
        xthread::Task A = taskflow.emplace([](){});
        xthread::Task B = taskflow.emplace([](){});
        xthread::Task C = taskflow.emplace([](){});
        ...
        */
        ...
        @note
        To know more about xthread::Semaphore, please refer to @ref LimitTheMaximumConcurrency.
        */
        ...
        */
        ...
        xthread::Taskflow taskflow("attach data to a task");
        ...
        */
        ...
        A placeholder task is created from xthread::Taskflow::placeholder and is associated with
        ...
        */
        ...
        @code{.cpp}
        xthread::Task task = taskflow.placeholder();
        ...
        */
        ...
        @tparam V a callable type (function, lambda, etc.) that accepts a xthread::Task handle
        ...
        task1.for_each_successor([](xthread::Task successor){
        ...
        */
        ...
        task1.for_each_predecessor([](xthread::Task predecessor){
        ...
        */
        ...
        @code{.cpp}
        xthread::Task task = taskflow.emplace([](xthread::Subflow& sf){
          xthread::Task stask1 = sf.emplace([](){}).name("stask1");
          xthread::Task stask2 = sf.emplace([](){}).name("stask2");
        });
        // Iterate tasks in the subflow and print each subflow task.
        task.for_each_subflow_task([](xthread::Task stask){
        ...
        */
        ...
        A task can be one of the types defined in xthread::TaskType and can be printed in
        a human-readable form using xthread::to_string.

        @code{.cpp}
        auto task = taskflow.emplace([](){}).name("task");
        std::cout << task.name() << " type=[" << xthread::to_string(task.type()) << "]\n";
        @endcode
        ...
        */
        ...
        @return C-styled pointer to the attached user data by xthread::Task::data(void* data)
...
namespace std {
    ...
    @brief hash specialization for std::hash<xthread::Task>
    */
    template<>
    struct hash<xthread::Task> {
        auto operator()(const xthread::Task &task) const noexcept {
    ...
    @brief hash specialization for std::hash<xthread::TaskView>
    */
    template<>
    struct hash<xthread::TaskView> {
        auto operator()(const xthread::TaskView &task_view) const noexcept {
