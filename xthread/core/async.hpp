...
#include <xthread/core/runtime.hpp>
...
namespace xthread {
    ...
    auto Executor::_async(P &&params, F &&f, Topology *tpg, Node *parent) {
        // async task with runtime: [] (xthread::Runtime&) -> void {}
    ...
    >
    xthread::AsyncTask Executor::silent_dependent_async(F &&func, Tasks &&... tasks) {
    ...
    >
    xthread::AsyncTask Executor::silent_dependent_async(
    ...
    >
    xthread::AsyncTask Executor::silent_dependent_async(F &&func, I first, I last) {
    ...
    auto Executor::_dependent_async(P &&params, F &&func, I first, I last, Topology *tpg, Node *parent) {
        ...
        // async with runtime: [] (xthread::Runtime&) -> void {}
        if constexpr (is_runtime_task_v<F>) {
            std::promise<void> p;
            auto fu{p.get_future()};
            ...
            AsyncTask task(animate(
                ...
                [p=MoC{std::move(p)}, f=std::forward<F>(func)](xthread::Runtime &rt, bool reentered) mutable {
                    if (!reentered) {
                        f(rt);
                    } else {
                        auto &eptr = rt._parent->_exception_ptr;
    ...
    inline void Executor::_process_dependent_async(
        Node *node, xthread::AsyncTask &task, size_t &num_predecessors
    ) {
