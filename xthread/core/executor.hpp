...
#include <xthread/core/observer.hpp>
#include <xthread/core/taskflow.hpp>
#include <xthread/core/async_task.hpp>
#include <xthread/core/freelist.hpp>
...
namespace xthread {
    ...
    An xthread::Executor manages a set of worker threads to run tasks
    ...
    xthread::Task A = taskflow.emplace([] () { std::cout << "This is TaskA\n"; });
    xthread::Task B = taskflow.emplace([] () { std::cout << "This is TaskB\n"; });
    xthread::Task C = taskflow.emplace([] () { std::cout << "This is TaskC\n"; });

    // Build precedence between tasks
    A.precede(B, C);

    xthread::Future<void> fu = executor.run(taskflow);
    ...
    @note
    To know more about xthread::Executor, please refer to @ref ExecuteTaskflow.
    */
    class Executor {
    ...
    public:
        ...
        via deriving an instance from xthread::WorkerInterface.
        ...
        */
        ...
        @param taskflow a xthread::Taskflow object

        @return a xthread::Future that holds the result of the execution

        This member function executes the given taskflow once and returns a xthread::Future
        object that eventually holds the result of the execution.

        @code{.cpp}
        xthread::Future<void> future = executor.run(taskflow);
        // do something else
        future.wait();
        @endcode
        ...
        */
        xthread::Future<void> run(Taskflow &taskflow);

        /**
        @brief runs a moved taskflow once

        @param taskflow a moved xthread::Taskflow object

        @return a xthread::Future that holds the result of the execution

        This member function executes a moved taskflow once and returns a xthread::Future
        object that eventually holds the result of the execution.
        The executor will take care of the lifetime of the moved taskflow.

        @code{.cpp}
        xthread::Future<void> future = executor.run(std::move(taskflow));
        // do something else
        future.wait();
        @endcode
        ...
        */
        ...
        This member function returns a xthread::Future object that
        eventually holds the result of the execution.

        @code{.cpp}
        xthread::Future<void> future = executor.run(taskflow, [](){ std::cout << "done"; });
        // do something else
        future.wait();
        @endcode
        ...
        */
        template<typename C>
        xthread::Future<void> run(Taskflow &taskflow, C &&callable);
        ...
        @code{.cpp}
        xthread::Future<void> future = executor.run(
        ...
        */
        ...
        This member function executes the given taskflow @c N times and returns a xthread::Future
        object that eventually holds the result of the execution.

        @code{.cpp}
        xthread::Future<void> future = executor.run_n(taskflow, 2);  // run taskflow 2 times
        // do something else
        future.wait();
        @endcode
        ...
        This member function is thread-safe.
        */
        xthread::Future<void> run_n(Taskflow &taskflow, size_t N);
        ...
        This member function executes a moved taskflow @c N times and returns a xthread::Future
        object that eventually holds the result of the execution.
        The executor will take care of the lifetime of the moved taskflow.

        @code{.cpp}
        xthread::Future<void> future = executor.run_n(
        ...
        */
        ...
        @param taskflow a xthread::Taskflow
        @param N number of runs
        @param callable a callable object to be invoked after this run
        ...
        */
        template<typename C>
        xthread::Future<void> run_n(Taskflow &taskflow, size_t N, C &&callable);

        /**
        @brief runs a moved taskflow for @c N times and then invokes a callback

        @param taskflow a moved xthread::Taskflow
        @param N number of runs
        @param callable a callable object to be invoked after this run
        ...
        */
        ...
        @code{.cpp}
        xthread::Future<void> future = executor.run_until(
        ...
        */
        template<typename P>
        xthread::Future<void> run_until(Taskflow &taskflow, P &&pred);
        ...
        */
        ...
        xthread::Future<void> run_until(Taskflow &taskflow, P &&pred, C &&callable);
        ...
        */
        ...
        @tparam T target type which has `xthread::Graph& T::graph()` defined
        ...
        @code{.cpp}
        xthread::Executor executor(2);
        xthread::Taskflow taskflow;
        std::array<xthread::Taskflow, 1000> others;
        ...
        @attention
        You must call xthread::Executor::corun from a worker of the calling executor
        or an exception will be thrown.
        */
        template<typename T>
        void corun(T &target);
        ...
        @attention
        You must call xthread::Executor::corun_until from a worker of the calling executor
        or an exception will be thrown.
        */
        template<typename P>
        ...
        */
        ...
        @code{.cpp}
        xthread::Executor executor(4);   // 4 workers in the executor
        ...
        */
        ...
        @tparam Observer observer type derived from xthread::ObserverInterface
        ...
        xthread::ObserverInterface::on_entry and xthread::ObserverInterface::on_exit
        ...
        */
        ...
        /**
        @brief similar to xthread::Executor::async but does not return a future object
        ...
        This member function is more efficient than xthread::Executor::async
        ...
        */
        ...
        @tparam F callable type
        @tparam Tasks task types convertible to xthread::AsyncTask

        @param func callable object
        @param tasks asynchronous tasks on which this execution depends

        @return a xthread::AsyncTask handle

        This member function is more efficient than xthread::Executor::dependent_async
        ...
        */
        ...
        >
        xthread::AsyncTask silent_dependent_async(F &&func, Tasks &&... tasks);
        ...
        @code{.cpp}
        xthread::AsyncTask A = executor.silent_dependent_async("A", [](){ printf("A\n"); });
        xthread::AsyncTask B = executor.silent_dependent_async("B", [](){ printf("B\n"); });
        ...
        */
        ...
        >
        xthread::AsyncTask silent_dependent_async(P &&params, F &&func, Tasks &&... tasks);
        ...
        @code{.cpp}
        std::array<xthread::AsyncTask, 2> array {
        ...
        */
        ...
        >
        xthread::AsyncTask silent_dependent_async(F &&func, I first, I last);
        ...
        */
        ...
        >
        xthread::AsyncTask silent_dependent_async(P &&params, F &&func, I first, I last);
        ...
        @return a pair of a xthread::AsyncTask handle and
                          a @std_future that holds the result of the execution

        The example below creates three asynchronous tasks, @c A, @c B, and @c C,
        in which task @c C runs after task @c A and task @c B.
        Task @c C returns a pair of its xthread::AsyncTask handle and a std::future<int>
        that eventually will hold the result of the execution.
        ...
        You can mix the use of xthread::AsyncTask handles
        returned by xthread::Executor::dependent_async and xthread::Executor::silent_dependent_async
    ...
    private:
        ...
        void _process_dependent_async(Node *, xthread::AsyncTask &, size_t &);
...
#define TF_INVOKE_CONTINUATION()  \
  if (cache) {                      \
    ...
    // Function: run
    inline xthread::Future<void> Executor::run(Taskflow &f) {
        return run_n(f, 1, []() {
        });
    }
    ...
    // Function: run
    template<typename C>
    xthread::Future<void> Executor::run(Taskflow &f, C &&c) {
    ...
    // Function: run_n
    inline xthread::Future<void> Executor::run_n(Taskflow &f, size_t repeat) {
    ...
    // Function: run_n
    template<typename C>
    xthread::Future<void> Executor::run_n(Taskflow &f, size_t repeat, C &&c) {
        return run_until(
    ...
    // Function: run_until
    template<typename P>
    xthread::Future<void> Executor::run_until(Taskflow &f, P &&pred) {
    ...
    // Function: run_until
    template<typename P, typename C>
    xthread::Future<void> Executor::run_until(Taskflow &f, P &&p, C &&c) {
        _increment_topology();
        ...
        if (f.empty() || p()) {
            ...
            promise.set_value();
            _decrement_topology();
            return xthread::Future<void>(promise.get_future());
        }
        ...
        xthread::Future<void> future(t->_promise.get_future(), t);
    ...
    // Function: run_until
    template<typename P, typename C>
    xthread::Future<void> Executor::run_until(Taskflow &&f, P &&pred, C &&c) {
