...
#include <xthread/core/task.hpp>
#include <xthread/algorithm/partitioner.hpp>
...
namespace xthread {
    ...
    from which xthread::Taskflow and xthread::Subflow are derived.
    ...
    */
    ...
    public:
        ...
        */
        ...
        @return a xthread::Task handle

        The following example creates a static task.

        @code{.cpp}
        xthread::Task static_task = taskflow.emplace([](){});
        @endcode
        ...
        */
        ...
        @tparam C callable type constructible from std::function<void(xthread::Runtime&)>
        ...
        @code{.cpp}
        xthread::Task static_task = taskflow.emplace([](xthread::Runtime&){});
        @endcode
        ...
        */
        ...
        @tparam C callable type constructible from std::function<void(xthread::Subflow&)>

        @param callable callable to construct a dynamic task

        @return a xthread::Task handle

        The following example creates a dynamic task (xthread::Subflow)
        that spawns two static tasks.

        @code{.cpp}
        xthread::Task dynamic_task = taskflow.emplace([](xthread::Subflow& sf){
          xthread::Task static_task1 = sf.emplace([](){});
          xthread::Task static_task2 = sf.emplace([](){});
        });
        @endcode
        ...
        */
        ...
        auto [init, cond, branch1, branch2, branch3] = taskflow.emplace(
          [] () { },
          [] () { return xthread::SmallVector{0, 2}; },
          [] () { std::cout << "branch1\n"; },
          [] () { std::cout << "branch2\n"; },
          [] () { std::cout << "branch3\n"; }
        );
        ...
        */
        ...
        xthread::Task C = taskflow.emplace([](){ std::cout << "C"; });
        xthread::Task D = taskflow.emplace([](){ std::cout << "D"; });
        A.precede(B, C, D);
        ...
        */
        ...
        @code{.cpp}
        xthread::Taskflow t1, t2;
        t1.emplace([](){ std::cout << "t1"; });

        // t2 is partially composed of t1
        xthread::Task comp = t2.composed_of(t1);
        xthread::Task init = t2.emplace([](){ std::cout << "t2"; });
        init.precede(comp);
        @endcode
        ...
        type xthread::Graph such that it can interact with the executor.
        For example:
        ...
        struct MyObj {
          xthread::Graph graph;
          MyObj() {
            xthread::FlowBuilder builder(graph);
            xthread::Task task = builder.emplace([](){
              std::cout << "a task\n";  // static task
            });
          }
        ...
        MyObj obj;
        xthread::Task comp = taskflow.composed_of(obj);
        @endcode
        ...
        */
        ...
        xthread::Task placeholder = taskflow.placeholder();
        ...
        */
        ...
        xthread::Task C = taskflow.emplace([](){ std::cout << "C"; });
        xthread::Task D = taskflow.emplace([](){ std::cout << "D"; });
        std::vector<xthread::Task> tasks {A, B, C, D}
        ...
        */
        ...
        @tparam P partitioner type (default xthread::DefaultPartitioner)
        ...
        */
        ...
        @tparam R index range type (xthread::IndexRange)
        @tparam C callable type
        @tparam P partitioner type (default xthread::DefaultPartitioner)
        ...
        @code{.cpp}
        // [0, 17) with a step size of 2 using xthread::IndexRange
        xthread::IndexRange<int> range(0, 17, 2);

        // parallelize the sequence [0, 2, 4, 6, 8, 10, 12, 14, 16]
        taskflow.for_each_by_index(range, [](xthread::IndexRange<int> range) {
        ...
        The callable needs to take a single argument of type xthread::IndexRange.
        ...
        */
        ...
        taskflow.reduce_by_index(
          xthread::IndexRange<size_t>(0, N, 1),
          // final result
          res,
          // local reducer
          [&](xthread::IndexRange<size_t> subrange, std::optional<int> running_total) -> int {
      ...
      */
        ...
        */
        ...
        @param part partitioning algorithm (default xthread::DefaultPartitioner)
    ...
    Task FlowBuilder::emplace(C &&c) {
        if constexpr (std::is_invocable_v<C, xthread::Runtime &>) {
            return Task(_graph._emplace_back(NSTATE::NONE, ESTATE::NONE, DefaultTaskParams{}, nullptr, nullptr, 0,
                                             std::in_place_type_t<Node::Runtime>{}, std::forward<C>(c)
            ));
        } else if constexpr (std::is_invocable_v<C, xthread::NonpreemptiveRuntime &>) {
            return Task(_graph._emplace_back(NSTATE::NONE, ESTATE::NONE, DefaultTaskParams{}, nullptr, nullptr, 0,
    ...
    xthread::Subflow is spawned from the execution of a task to dynamically manage a
    child graph that may depend on runtime variables.
    You can explicitly join a subflow by calling xthread::Subflow::join, respectively.
