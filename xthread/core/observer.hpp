...
#include <xthread/core/worker.hpp>
...
namespace xthread {
    ...
    */
    ...
    The xthread::ObserverInterface class allows users to define custom methods to monitor
    ...
    xthread::ObserverInterface provides immutable wrappers,
    xthread::WorkerView and xthread::TaskView, over workers and tasks.

    Please refer to xthread::WorkerView and xthread::TaskView for details.

    Example usage:

    @code{.cpp}

    struct MyObserver : public xthread::ObserverInterface {
      ...
      void on_entry(WorkerView w, xthread::TaskView tv) override final {
        std::ostringstream oss;
        oss << "worker " << w.id() << " ready to run " << tv.name() << '\n';
        std::cout << oss.str();
      }

      void on_exit(WorkerView w, xthread::TaskView tv) override final {
        std::ostringstream oss;
    ...
    */
    ...
    A xthread::ChromeObserver inherits xthread::ObserverInterface and defines methods to dump
    ...
    // create a custom observer
    std::shared_ptr<xthread::ChromeObserver> observer = executor.make_observer<xthread::ChromeObserver>();
    ...
    */
    ...
    A xthread::TFProfObserver inherits xthread::ObserverInterface and defines methods to dump
    the observed thread activities into a format that can be visualized through
    @TFProf.
    ...
    // create a custom observer
    std::shared_ptr<xthread::TFProfObserver> observer = executor.make_observer<xthread::TFProfObserver>();
