...
#include <xthread/core/flow_builder.h>

/**
@file taskflow/core/taskflow.h
...
namespace xthread {
    ...
    dependency between two tasks. A task is one of the following types:
      ...
      2. subflow task        : the callable constructible from
                               @c std::function<void(xthread::Subflow&)>
      3. condition task      : the callable constructible from
                               @c std::function<int()>
      4. multi-condition task: the callable constructible from
                               @c %std::function<xthread::SmallVector<int>()>
      5. module task         : the task constructed from xthread::Taskflow::composed_of
                               @c std::function<void(xthread::Runtime&)>
    ...
    xthread::Task A = taskflow.emplace([](){ std::cout << "TaskA\n"; });
    xthread::Task B = taskflow.emplace([](){ std::cout << "TaskB\n"; });
    xthread::Task C = taskflow.emplace([](){ std::cout << "TaskC\n"; });
    xthread::Task D = taskflow.emplace([](){ std::cout << "TaskD\n"; });
    ...
    */
    ...
    public:
        ...
        @code{.cpp}
        xthread::Taskflow taskflow("My Taskflow");
        ...
        */
        ...
        @code{.cpp}
        xthread::Taskflow taskflow1(std::move(taskflow2));
        ...
        */
        ...
        @code{.cpp}
        xthread::Task parent = taskflow.emplace([](xthread::Subflow sf){
        ...
        */
        ...
        This method is similar to xthread::Taskflow::dump(std::ostream& ostream),
        ...
        */
        ...
        Tasks that are created dynamically, such as those via xthread::Subflow, are not counted.
        ...
        my_task.work([](xthread::Subflow& sf){
          sf.emplace(
            [](){ std::cout << "Task A\n"; },
            [](){ std::cout << "Task B\n"; },
            [](){ std::cout << "Task C\n"; },
        ...
        */
        ...
        An empty taskflow has no tasks, i.e., the return of xthread::Taskflow::num_tasks is `0`.
        ...
        */
        ...
        A visitor is a callable that takes an argument of type xthread::Task
        and returns nothing. The following example iterates each task in a
        taskflow and prints its name:

        @code{.cpp}
        taskflow.for_each_task([](xthread::Task task){
          std::cout << task.name() << '\n';
        });
        @endcode
        */
        ...
        */
        ...
        A graph object is of type xthread::Graph and stores a task dependency graph that can be executed
        by an xthread::Executor.
    ...
    xthread::Future is a derived class from std::future that will eventually hold the
    execution result of a submitted taskflow (xthread::Executor::run series).
    In addition to the base methods inherited from std::future,
    you can call xthread::Future::cancel to cancel the execution of the running taskflow
    ...
    // submit the taskflow
    xthread::Future fu = executor.run(taskflow);
    ...
    */
    ...
    public:
        ...
        */
        ...
        You can call xthread::Future::wait to wait for the cancellation to complete.
