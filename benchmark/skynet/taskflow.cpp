...
size_t skynet_one_tf(xthread::Runtime& rt, size_t BaseNum, size_t Depth, size_t MaxDepth) {
  ...
  for (size_t i = 0; i < 10; ++i) {
    rt.silent_async([=, &results](xthread::Runtime& s) {
...
void skynet(size_t num_threads, size_t MaxDepth) {
  ...
  executor.async([=](xthread::Runtime& rt) { skynet_one_tf(rt, 0, 0, MaxDepth); }).wait();
}
