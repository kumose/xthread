...
auto spawn_async(int j, std::vector<char>&a, xthread::Runtime& rt) -> int {
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (queens_ok(j + 1, buf[i].data())) {
      rt.silent_async([&parts, &buf, i, j](xthread::Runtime& rt1) {
...
int nqueens_taskflow(int i, size_t num_threads, std::vector<char>& buf) {
  ...
  executor.async([i, &buf, &output](xthread::Runtime& rt){
