...
xthread::Executor& get_executor() {
  static xthread::Executor executor;
  return executor;
}

// integrate computation 
auto spawn_async(double x1, double y1, double x2, double y2, double area, xthread::Runtime& rt) {
  ...
  rt.silent_async([x1,y1,x0,y0,&area_x1x0](xthread::Runtime& rt1){
...
auto integrate_taskflow(size_t num_threads, double x1, double y1, double x2, double y2) {
  ...
  executor.async([x1, y1, x2, y2, &area](xthread::Runtime& rt){
